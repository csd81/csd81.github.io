...
`,Cn=`program back_substitution_demo
  implicit none
...
`,Fn=`function back_substitution(U, b)
    n = length(b)
    x = zeros(n)
    for i in n:-1:1
...
`,Vn=`function backSubstitution(U, b) {
  const n = b.length;
...
`,Dn=`function x = back_substitution(U, b)
...
`,Rn=`back_substitution <- function(U, b) {
  # Solve an upper-triangular system U x = b.
  n <- length(b)
  x <- numeric(n)
  for (i in n:1) {
...
b <- c(1, 8, 9)
print(back_substitution(U, b))    # [1, 2, 3]
`,Ln=`// Solve an upper-triangular system U x = b.
...
`,Xn=`backSubstitution[U_, b_] := Module[{n = Length[b], x},
   x = ConstantArray[0., n];
...
`,Qn=`program gauss_complete_demo
  implicit none
  integer, parameter :: n = 3
...
`,Zn=`function gauss_complete_pivot(A, b)
...
`,ne=`// Gaussian elimination with complete (row + column) pivoting.
...
`,ee=`function x = gauss_complete_pivot(A, b)
...
`,ie=`gauss_complete_pivot <- function(A, b) {
  # Gaussian elimination with complete (row+column) pivoting.
  # Returns x (column swaps are undone before returning).
...
`,ae=`// Gaussian elimination with complete (row + column) pivoting.
...
`,oe=`gaussComplete[Ain_, bin_] := Module[
...
`,re=`program gauss_elimination_demo
  implicit none
  integer, parameter :: n = 3
...
`,me=`function gauss_elimination(A, b)
    n = length(b)
    M = [A b]                       # augmented [A | b]
...
`,he=`function gaussElimination(A, b) {
  const n = b.length;
...
`,de=`function x = gauss_elimination(A, b)
...
`,ce=`gauss_elimination <- function(A, b) {
  # Naive Gaussian elimination (no pivoting) + back-substitution.
  n <- length(b)
...
b <- c(8, -11, -3)
print(gauss_elimination(A, b))    # [2, 3, -1]
`,fe=`// Naive Gaussian elimination (no pivoting) + back-substitution.
...
`,pe=`gaussElimination[A_, b_] := Module[{n = Length[b], M, x, f},
...
`,ke=`// Solve A x = b by Gauss-Jordan elimination.
...
`,ve=`function x = gauss_jordan(A, b)
...
`,ye=`// Solve A x = b by Gauss-Jordan elimination (reduced row echelon form).
...
`,we=`gaussJordan[Ain_, bin_] := Module[{M = N@MapThread[Append, {Ain, bin}], n = Length[bin], p, f},
   Do[
...
`,Ie=`function gauss_partial_pivot(A, b)
...
`,Te=`// Gaussian elimination with partial (row) pivoting.
...
`,Me=`function x = gauss_partial_pivot(A, b)
...
`,Se=`gauss_partial_pivot <- function(A, b) {
  # Solve A x = b by Gaussian elimination with partial (row) pivoting.
  A <- matrix(as.numeric(A), nrow = nrow(A))
  b <- as.numeric(b)
  n <- length(b)
...
`,We=`// Gaussian elimination with partial (row) pivoting.
...
`,Pe=`gaussPartial[Ain_, bin_] := Module[{A = N[Ain], b = N[bin], n = Length[bin], p, f, x},
   Do[
...
`,Be=`program matrix_inverse_demo
  implicit none
  integer, parameter :: n = 3
...
println(inverse(A))
`,Je=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
function inverse(A) {
  const n = A.length;
...
`,Ce=`function Ainv = matrix_inverse(A)
...
`,Ve=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
...
`,De=`inverse[Ain_] := Module[{A = N[Ain], n = Length[Ain], M, p, d},
...
`,Ke=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
d <- c(2, 4, 6, 13)
print(thomas(a, b, c, d))
`,nt=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
`,et=`thomas[a_, b_, cIn_, dIn_] := Module[{n = Length[dIn], c = cIn, d = dIn, x, m},
   c[[1]] /= b[[1]]; d[[1]] /= b[[1]];
   Do[m = b[[i]] - a[[i]] c[[i - 1]];
      If[i < n, c[[i]] /= m];
...
`,tt=Object.assign({"./back-substitution.cpp":Jn,"./back-substitution.f90":Cn,"./back-substitution.go":Hn,"./back-substitution.jl":Fn,"./back-substitution.js":Vn,"./back-substitution.m":Dn,"./back-substitution.py":Un,"./back-substitution.r":Rn,"./back-substitution.rs":Ln,"./back-substitution.wl":Xn,"./gauss-complete.cpp":Kn,"./gauss-complete.f90":Qn,"./gauss-complete.go":Yn,"./gauss-complete.jl":Zn,"./gauss-complete.js":ne,"./gauss-complete.m":ee,"./gauss-complete.py":te,"./gauss-complete.r":ie,"./gauss-complete.rs":ae,"./gauss-complete.wl":oe,"./gauss-elimination.cpp":se,"./gauss-elimination.f90":re,"./gauss-elimination.go":le,"./gauss-elimination.jl":me,"./gauss-elimination.js":he,"./gauss-elimination.m":de,"./gauss-elimination.py":ue,"./gauss-elimination.r":ce,"./gauss-elimination.rs":fe,"./gauss-elimination.wl":pe,"./gauss-jordan.cpp":be,"./gauss-jordan.f90":$e,"./gauss-jordan.go":ge,"./gauss-jordan.jl":xe,"./gauss-jordan.js":ke,"./gauss-jordan.m":ve,"./gauss-jordan.py":_e,"./gauss-jordan.r":Ae,"./gauss-jordan.rs":ye,"./gauss-jordan.wl":we,"./gauss-partial.cpp":ze,"./gauss-partial.f90":je,"./gauss-partial.go":qe,"./gauss-partial.jl":Ie,"./gauss-partial.js":Te,"./gauss-partial.m":Me,"./gauss-partial.py":Ge,"./gauss-partial.r":Se,"./gauss-partial.rs":We,"./gauss-partial.wl":Pe,"./matrix-inverse.cpp":Ee,"./matrix-inverse.f90":Be,"./matrix-inverse.go":Oe,"./matrix-inverse.jl":Ne,"./matrix-inverse.js":Je,"./matrix-inverse.m":Ce,"./matrix-inverse.py":He,"./matrix-inverse.r":Fe,"./matrix-inverse.rs":Ve,"./matrix-inverse.wl":De,"./tridiagonal.cpp":Ue,"./tridiagonal.f90":Re,"./tridiagonal.go":Le,"./tridiagonal.jl":Xe,"./tridiagonal.js":Ke,"./tridiagonal.m":Qe,"./tridiagonal.py":Ye,"./tridiagonal.r":Ze,"./tridiagonal.rs":nt,"./tridiagonal.wl":et}),M=(n,e)=>tt[`./${n}.${e}`],it={"back-substitution":{en:"Back-substitution (upper-triangular system)",hu:"Visszahelyettesítés (felső háromszögű rendszer)"},"gauss-elimination":{en:"Gaussian elimination (no pivoting)",hu:"Gauss-elimináció (főelemkiválasztás nélkül)"},"gauss-partial":{en:"Gaussian elimination — partial pivoting",hu:"Gauss-elimináció — részleges főelemkiválasztás"},"gauss-complete":{en:"Gaussian elimination — complete pivoting",hu:"Gauss-elimináció — teljes főelemkiválasztás"},"gauss-jordan":{en:"Gauss–Jordan elimination",hu:"Gauss–Jordan-elimináció"},"matrix-inverse":{en:"Matrix inverse (Gauss–Jordan on [A | I])",hu:"Mátrixinverz (Gauss–Jordan az [A | I] mátrixon)"},tridiagonal:{en:"Tridiagonal solver (Thomas algorithm)",hu:"Tridiagonális megoldó (Thomas-algoritmus)"}},at=n=>({id:n,caption:it[n],snippets:{matlab:M(n,"m"),python:M(n,"py"),cpp:M(n,"cpp"),julia:M(n,"jl"),rust:M(n,"rs"),fortran:M(n,"f90"),wolfram:M(n,"wl"),javascript:M(n,"js"),go:M(n,"go"),r:M(n,"r")}}),ot={s33:["back-substitution","gauss-elimination","gauss-partial","gauss-complete","gauss-jordan","matrix-inverse","tridiagonal"]};function st(n){return(ot[n]??[]).map(at)}function Y(n,e){return new sn(n)}const L=new sn(0),bn=new sn(1);function S(n){return n.n===0}function an(n){if(n.n===0)return"0";const e=n.s<0?"-":"";return n.d===1?`${e}${n.n}`:`${e}\\frac{${n.n}}{${n.d}}`}function j(n){if(n.n===0)return"0";const e=n.s<0?"-":"";return n.d===1?`${e}${n.n}`:`${e}${n.n}/${n.d}`}function X(n){return n.map(e=>e.slice())}function rt(n){const e=[];for(let a=0;a<n;a++)e.push(Array.from({length:n},(i,s)=>Y(a===s?1:0)));return e}function lt(n,e){return n.map((a,i)=>a.concat(e[i]))}const mt=n=>n.length?n[0].length:0;function ht(n,e,a){return n.map(i=>i.slice(e,a))}function $(n){return n+1}function rn(n,e,a){const i=X(n),s=i.length,h=mt(i),m=Array.from({length:e},(r,z)=>z),c=[],o=[];let d=0,k=0,g=!1;const p=()=>X(i);o.push({kind:"init",matrix:p(),coeffCols:e,varOrder:m.slice(),caption:{en:"Initial augmented matrix.",hu:"Kiindulási kibővített mátrix."}});const T=r=>{let z=L;for(let I=0;I<e;I++){const u=i[r][I].abs();u.compare(z)>0&&(z=u)}return z},q=Math.min(s,e);for(let r=0;r<q;r++){if(a.pivoting==="partial"){let u=r;for(let v=r+1;v<s;v++)i[v][r].abs().compare(i[u][r].abs())>0&&(u=v);!S(i[u][r])&&u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,o.push({kind:"row-swap",matrix:p(),coeffCols:e,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Partial pivoting: swap rows ${$(r)} and ${$(u)} (largest |entry| in column ${$(r)}).`,hu:`Részleges főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje (legnagyobb |elem| a(z) ${$(r)}. oszlopban).`}}))}else if(a.pivoting==="scaled"){let u=r,v=null;for(let w=r;w<s;w++){const y=T(w),P=S(y)?L:i[w][r].abs().div(y);(v===null||P.compare(v)>0)&&(v=P,u=w)}!S(i[u][r])&&u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,o.push({kind:"row-swap",matrix:p(),coeffCols:e,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Scaled partial pivoting: swap rows ${$(r)} and ${$(u)} (largest scaled ratio).`,hu:`Skálázott főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje (legnagyobb skálázott hányados).`}}))}else if(a.pivoting==="complete"){let u=r,v=r;for(let w=r;w<s;w++)for(let y=r;y<e;y++)i[w][y].abs().compare(i[u][v].abs())>0&&(u=w,v=y);if(!S(i[u][v])&&(u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,o.push({kind:"row-swap",matrix:p(),coeffCols:e,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Complete pivoting: swap rows ${$(r)} and ${$(u)}.`,hu:`Teljes főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje.`}})),v!==r)){for(let w=0;w<s;w++)[i[w][r],i[w][v]]=[i[w][v],i[w][r]];[m[r],m[v]]=[m[v],m[r]],k++,o.push({kind:"col-swap",matrix:p(),coeffCols:e,varOrder:m.slice(),swapCols:[r,v],caption:{en:`Complete pivoting: swap columns ${$(r)} and ${$(v)} (track variable order).`,hu:`Teljes főelemkiválasztás: ${$(r)}. és ${$(v)}. oszlop cseréje (a változók sorrendjét követjük).`}})}}const z=i[r][r];if(c.push(z),S(z)){g=!0,o.push({kind:"pivot-select",matrix:p(),coeffCols:e,varOrder:m.slice(),pivot:[r,r],caption:{en:`Pivot a_{${$(r)}${$(r)}} = 0 and no nonzero entry is available — the elimination stops (matrix is singular${a.pivoting==="none"?" for this strategy":""}).`,hu:`A főelem a_{${$(r)}${$(r)}} = 0, és nincs nemnulla elem — az elimináció megáll (a mátrix szinguláris${a.pivoting==="none"?" ennél a stratégiánál":""}).`}});break}o.push({kind:"pivot-select",matrix:p(),coeffCols:e,varOrder:m.slice(),pivot:[r,r],caption:{en:`Pivot a_{${$(r)}${$(r)}} = ${j(z)}.`,hu:`Főelem a_{${$(r)}${$(r)}} = ${j(z)}.`}});const I=a.method==="gauss-jordan"?0:r+1;for(let u=I;u<s;u++){if(u===r||S(i[u][r]))continue;const v=i[u][r].div(z),w=[];for(let y=r;y<h;y++){const P=i[u][y];i[u][y]=i[u][y].sub(v.mul(i[r][y])),i[u][y].equals(P)||w.push([u,y])}o.push({kind:"eliminate",matrix:p(),coeffCols:e,varOrder:m.slice(),pivot:[r,r],multiplier:v,changed:w,caption:{en:`Eliminate: R${$(u)} ← R${$(u)} − (${j(v)})·R${$(r)}.`,hu:`Kiküszöbölés: S${$(u)} ← S${$(u)} − (${j(v)})·S${$(r)}.`}})}}if(a.method==="gauss-jordan"&&!g)for(let r=0;r<q;r++){const z=i[r][r];if(S(z)||z.equals(bn))continue;const I=[];for(let u=0;u<h;u++)i[r][u]=i[r][u].div(z),I.push([r,u]);o.push({kind:"normalize",matrix:p(),coeffCols:e,varOrder:m.slice(),pivot:[r,r],changed:I,caption:{en:`Normalise: R${$(r)} ← R${$(r)} / (${j(z)}).`,hu:`Normálás: S${$(r)} ← S${$(r)} / (${j(z)}).`}})}return{steps:o,matrix:i,coeffCols:e,varOrder:m,pivots:c,rowSwaps:d,colSwaps:k,singular:g}}function $n(n){if(n.coeffCols!==n.matrix.length)return null;if(n.singular)return L;let e=bn;for(const a of n.pivots)e=e.mul(a);return(n.rowSwaps+n.colSwaps)%2===1&&(e=e.neg()),e}function dt(n){const{matrix:e,coeffCols:a,varOrder:i,singular:s}=n;if(s)return{steps:[],solution:null};const h=a,m=new Array(a),c=[];for(let d=a-1;d>=0;d--){let k=e[d][h];for(let p=d+1;p<a;p++)k=k.sub(e[d][p].mul(m[p]));m[d]=k.div(e[d][d]);const g=i[d]+1;c.push({kind:"back-sub",matrix:X(e),coeffCols:a,varOrder:i.slice(),pivot:[d,d],caption:{en:`Back-substitution: x${g} = ${j(m[d])}.`,hu:`Visszahelyettesítés: x${g} = ${j(m[d])}.`}})}const o=new Array(a);for(let d=0;d<a;d++)o[i[d]]=m[d];return{steps:c,solution:o}}function ut(n,e){return n.map((a,i)=>a.concat([e[i]]))}function ct(n,e,a){const i=n[0].length,s=ut(n,e),h=rn(s,i,a),m=$n(h);if(h.singular)return{steps:h.steps,solution:null,singular:!0,determinant:m};let c=h.steps,o;if(a.method==="gauss"){const k=dt(h);c=c.concat(k.steps),o=k.solution}else{const k=i;o=new Array(i);for(let g=0;g<i;g++)o[h.varOrder[g]]=h.matrix[g][k]}const d=o.map((k,g)=>`x${g+1} = ${j(k)}`).join(",  ");return c=c.concat([{kind:"done",matrix:X(h.matrix),coeffCols:i,varOrder:h.varOrder.slice(),caption:{en:`Solution: ${d}.`,hu:`Megoldás: ${d}.`}}]),{steps:c,solution:o,singular:!1,determinant:m}}function ft(n,e="none"){const a=n.length,i=lt(n,rt(a)),s=rn(i,a,{method:"gauss-jordan",pivoting:e});if(s.singular)return{steps:s.steps,inverse:null,singular:!0};const h=ht(s.matrix,a,2*a);return{steps:s.steps.concat([{kind:"done",matrix:X(s.matrix),coeffCols:a,varOrder:s.varOrder.slice(),caption:{en:"The right block is A⁻¹.",hu:"A jobb oldali blokk az A⁻¹."}}]),inverse:h,singular:!1}}function pt(n,e="partial"){const a=n.length,i=rn(n.map(m=>m.slice()),a,{method:"gauss",pivoting:e}),s=$n(i);return{steps:i.steps,determinant:s}}function D(n,e,a,i,s){const h=e.length,m=[];for(let c=0;c<h;c++){const o=new Array(h+1).fill(L);o[c]=e[c],c<h-1&&(o[c+1]=a[c]),c>0&&(o[c-1]=c<=s?L:n[c-1]),o[h]=i[c],m.push(o)}return m}function bt(n){const e=n.d.length,a=n.a.slice(),i=n.d.slice(),s=n.c.slice(),h=n.b.slice(),m=[];m.push({kind:"init",matrix:D(a,i,s,h,0),coeffCols:e,varOrder:Array.from({length:e},(o,d)=>d),caption:{en:"Initial tridiagonal system (A | b).",hu:"Kiindulási tridiagonális rendszer (A | b)."}});for(let o=1;o<e;o++){if(S(i[o-1]))return m.push({kind:"pivot-select",matrix:D(a,i,s,h,o-1),coeffCols:e,varOrder:Array.from({length:e},(k,g)=>g),pivot:[o-1,o-1],caption:{en:`Zero pivot d${o} — the Thomas algorithm cannot continue without pivoting.`,hu:`Nulla főelem d${o} — a Thomas-algoritmus főelemkiválasztás nélkül nem folytatható.`}}),{steps:m,solution:null,singular:!0};const d=a[o-1].div(i[o-1]);i[o]=i[o].sub(d.mul(s[o-1])),h[o]=h[o].sub(d.mul(h[o-1])),m.push({kind:"eliminate",matrix:D(a,i,s,h,o),coeffCols:e,varOrder:Array.from({length:e},(k,g)=>g),pivot:[o-1,o-1],multiplier:d,changed:[[o,o-1],[o,o],[o,e]],caption:{en:`Eliminate a${o}: m = a${o}/d${o} = ${j(d)}; update d${o+1} and b${o+1}.`,hu:`a${o} kiküszöbölése: m = a${o}/d${o} = ${j(d)}; d${o+1} és b${o+1} frissítése.`}})}if(S(i[e-1]))return{steps:m,solution:null,singular:!0};const c=new Array(e);c[e-1]=h[e-1].div(i[e-1]),m.push({kind:"back-sub",matrix:D(a,i,s,h,e-1),coeffCols:e,varOrder:Array.from({length:e},(o,d)=>d),pivot:[e-1,e-1],caption:{en:`Back-substitution: x${e} = b${e}/d${e} = ${j(c[e-1])}.`,hu:`Visszahelyettesítés: x${e} = b${e}/d${e} = ${j(c[e-1])}.`}});for(let o=e-2;o>=0;o--)c[o]=h[o].sub(s[o].mul(c[o+1])).div(i[o]),m.push({kind:"back-sub",matrix:D(a,i,s,h,e-1),coeffCols:e,varOrder:Array.from({length:e},(d,k)=>k),pivot:[o,o],caption:{en:`Back-substitution: x${o+1} = (b${o+1} − c${o+1}·x${o+2})/d${o+1} = ${j(c[o])}.`,hu:`Visszahelyettesítés: x${o+1} = (b${o+1} − c${o+1}·x${o+2})/d${o+1} = ${j(c[o])}.`}});return{steps:m,solution:c,singular:!1}}const on=[{id:"ex3-22",ref:"Example 3.22",name:{en:"Gaussian elimination",hu:"Gauss-elimináció"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[-11,-8,27,28],note:{en:"Solves cleanly without pivoting; solution (-3, 2, 4, -2).",hu:"Főelemkiválasztás nélkül is megoldható; megoldás (-3, 2, 4, -2)."}},{id:"ex3-24",ref:"Example 3.24 / 3.27",name:{en:"Zero pivot (needs pivoting)",hu:"Nulla főelem (kell csere)"},A:[[2,-1,0,-3],[2,-1,1,5],[-3,1,1,-2],[2,4,0,-1]],b:[8,2,-5,21],note:{en:"Without pivoting it stalls on a zero pivot. Switch pivoting to Partial to finish — solution (4, 3, 2, -1).",hu:"Főelemkiválasztás nélkül nulla főelemen elakad. Válts Részleges főelemkiválasztásra — megoldás (4, 3, 2, -1)."}},{id:"ex3-25",ref:"Example 3.25",name:{en:"Ill-conditioned 2×2",hu:"Rosszul kondicionált 2×2"},A:[["1/5000","-61/2"],["253/50","-21/20"]],b:["-6099/100","2509/10"],note:{en:"Exact solution (50, 2). With finite-digit arithmetic, dividing by the tiny pivot 0.0002 blows up the error — partial pivoting fixes it.",hu:"Pontos megoldás (50, 2). Véges jegyű aritmetikával a kicsi 0.0002 főelemmel való osztás felnagyítja a hibát — a részleges főelemkiválasztás megoldja."}},{id:"ex3-33a",ref:"§3.3 Exercise 1(a)",name:{en:"Exercise 3×3",hu:"Feladat 3×3"},A:[[2,2,-2],[-1,3,0],[4,2,-3]],b:[-4,-11,-1]}],$t=[{id:"ex3-38",ref:"Example 3.38",name:{en:"Invert a 3×3",hu:"3×3 invertálása"},A:[[1,0,2],[-1,1,0],[-2,0,-1]],b:[0,0,0],note:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]].",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]]."}},{id:"ex3-39",ref:"Example 3.39",name:{en:"Determinant = 114",hu:"Determináns = 114"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[0,0,0,0],note:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"ex3-7-1a",ref:"§3.7 Exercise 1(a)",name:{en:"Invert 3×3 (exercise)",hu:"3×3 invertálása (feladat)"},A:[[-1,1,2],[-2,1,0],[0,1,-1]],b:[0,0,0]}],B=[{id:"ex3-5-1",ref:"§3.5 Exercise 1",name:{en:"6×6 tridiagonal",hu:"6×6 tridiagonális"},a:["1/2","1/2","1/2","1/2","1/2"],d:[1,4,2,4,2,1],c:["-1/2","-1/2","-1/2","-1/2","-1/2"],b:["3/2",-4,2,-4,2,"-1/2"]},{id:"tri-small",ref:"Warm-up",name:{en:"4×4 tridiagonal",hu:"4×4 tridiagonális"},a:[-1,-1,-1],d:[4,4,4,4],c:[-1,-1,-1],b:[5,5,10,23]}];function gt(n,e,a){return!!n&&n[0]===e&&n[1]===a}function gn({matrix:n,coeffCols:e,pivot:a,changed:i,varOrder:s,bracket:h=!0}){const m=new Set((i??[]).map(([o,d])=>`${o},${d}`)),c=s!==void 0&&s.some((o,d)=>o!==d);return t.jsxs("div",{className:"matrix-view",children:[t.jsx("table",{className:`matrix${h?" bracketed":""}`,children:t.jsx("tbody",{children:n.map((o,d)=>t.jsx("tr",{children:o.map((k,g)=>{const p=[];return gt(a,d,g)?p.push("cell-pivot"):m.has(`${d},${g}`)&&p.push("cell-changed"),e!==void 0&&g===e&&p.push("rhs-sep"),t.jsx("td",{className:p.join(" ")||void 0,children:t.jsx(J,{tex:an(k)})},g)})},d))})}),c&&t.jsx("div",{className:"var-order",children:s.map((o,d)=>t.jsxs("span",{children:["col ",d+1," → ",t.jsx(J,{tex:`x_{${o+1}}`})]},d))})]})}function xt({values:n,onChange:e,label:a,invalid:i}){var h;const s=((h=n[0])==null?void 0:h.length)??0;return t.jsxs("label",{className:"field",children:[a,t.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:`repeat(${s}, auto)`},children:n.map((m,c)=>m.map((o,d)=>t.jsx("input",{type:"text",inputMode:"decimal",value:o,"aria-label":`a${c+1}${d+1}`,style:i!=null&&i(c,d)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:k=>e(c,d,k.target.value)},`${c}-${d}`)))})]})}function U({values:n,onChange:e,label:a,invalid:i,horizontal:s=!1}){return t.jsxs("label",{className:"field",children:[a,t.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:s?`repeat(${n.length}, auto)`:"auto"},children:n.map((h,m)=>t.jsx("input",{type:"text",inputMode:"decimal",value:h,"aria-label":`b${m+1}`,style:i!=null&&i(m)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:c=>e(m,c.target.value)},m))})]})}const dn=n=>n.map(e=>e.map(String)),G=n=>n.map(String),xn=n=>Array.from({length:n},()=>"0"),kt=n=>Array.from({length:n},()=>xn(n)),F=n=>n.map(e=>Y(e.trim()===""?0:e.trim())),vt=n=>n.map(F);function R(n){return n==="solve"?on:$t}function _t({initial:n}){const{t:e,pick:a,lang:i}=W(),[s,h]=A.useState((n==null?void 0:n.mode)??"solve"),[m,c]=A.useState((n==null?void 0:n.method)??"gauss"),[o,d]=A.useState((n==null?void 0:n.pivot)??"none"),k=A.useMemo(()=>{const l=(n==null?void 0:n.mode)??"solve";if(l==="tridiagonal")return{presetId:(B.find(_=>_.id===(n==null?void 0:n.presetId))??B[0]).id};const x=R(l);return{presetId:(x.find(b=>b.id===(n==null?void 0:n.presetId))??x[0]).id}},[n==null?void 0:n.mode,n==null?void 0:n.presetId]),[g,p]=A.useState(k.presetId),T=on[0],[q,r]=A.useState(()=>{const l=(n==null?void 0:n.mode)??"solve",x=R(l),f=x.find(b=>b.id===(n==null?void 0:n.presetId))??x[0]??T;return dn(f.A)}),[z,I]=A.useState(()=>{const l=on.find(x=>x.id===(n==null?void 0:n.presetId))??T;return G(l.b)}),[u,v]=A.useState(()=>{const l=B.find(x=>x.id===(n==null?void 0:n.presetId))??B[0];return{a:G(l.a),d:G(l.d),c:G(l.c),b:G(l.b)}}),[w,y]=A.useState(0),[P,Z]=A.useState(!1);function nn(l,x){if(p(x),l==="tridiagonal"){const _=B.find(H=>H.id===x)??B[0];v({a:G(_.a),d:G(_.d),c:G(_.c),b:G(_.b)});return}const f=R(l),b=f.find(_=>_.id===x)??f[0];b&&(r(dn(b.A)),I(G(b.b)))}function kn(l){h(l),l==="inverse"?(c("gauss-jordan"),(o==="complete"||o==="scaled")&&d("none")):l==="determinant"&&c("gauss"),l==="tridiagonal"?nn(l,B[0].id):nn(l,R(l)[0].id)}function vn(l){r(x=>{const f=kt(l);for(let b=0;b<Math.min(l,x.length);b++)for(let _=0;_<Math.min(l,x[b].length);_++)f[b][_]=x[b][_];return f}),I(x=>{const f=xn(l);for(let b=0;b<Math.min(l,x.length);b++)f[b]=x[b];return f}),p("custom")}const C=A.useMemo(()=>{try{if(s==="tridiagonal"){const f=F(u.a),b=F(u.d),_=F(u.c),H=F(u.b);if(f.length!==b.length-1||_.length!==b.length-1||H.length!==b.length)return{steps:[],error:"dimension mismatch"};const V=bt({a:f,d:b,c:_,b:H});return{steps:V.steps,error:null,solution:V.solution,singular:V.singular}}const l=vt(q);if(s==="solve"){const f=ct(l,F(z),{method:m,pivoting:o});return{steps:f.steps,error:null,solution:f.solution,determinant:f.determinant,singular:f.singular}}if(s==="inverse"){const f=ft(l,o==="partial"?"partial":"none");return{steps:f.steps,error:null,inverse:f.inverse,singular:f.singular}}const x=pt(l,o);return{steps:x.steps,error:null,determinant:x.determinant}}catch{return{steps:[],error:"invalid"}}},[s,m,o,q,z,u]);A.useEffect(()=>{y(0),Z(!1)},[C]);const en=A.useRef(null);A.useEffect(()=>{if(P)return en.current=window.setInterval(()=>{y(l=>l>=C.steps.length-1?(Z(!1),l):l+1)},900),()=>{en.current&&window.clearInterval(en.current)}},[P,C.steps.length]);const E=C.steps,O=Math.min(w,Math.max(0,E.length-1)),N=E[O],_n=q.length,ln=s==="solve",mn=s==="solve"||s==="determinant"||s==="inverse",An=s==="inverse"?["none","partial"]:["none","partial","complete","scaled"],hn=s==="tridiagonal"?B:R(s),K=hn.find(l=>l.id===g)??null;return t.jsxs("div",{className:"grid-2",children:[t.jsxs("div",{className:"card stack",children:[t.jsxs("div",{className:"row",children:[t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.mode"),t.jsxs("select",{value:s,onChange:l=>kn(l.target.value),children:[t.jsx("option",{value:"solve",children:e("lab.mode.solve")}),t.jsx("option",{value:"inverse",children:e("lab.mode.inverse")}),t.jsx("option",{value:"determinant",children:e("lab.mode.determinant")}),t.jsx("option",{value:"tridiagonal",children:e("lab.mode.tridiagonal")})]})]}),t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.preset"),t.jsxs("select",{value:g,onChange:l=>nn(s,l.target.value),children:[hn.map(l=>t.jsxs("option",{value:l.id,children:[l.ref," — ",a(l.name)]},l.id)),t.jsx("option",{value:"custom",children:e("lab.preset.custom")})]})]})]}),(ln||mn)&&t.jsxs("div",{className:"row",children:[ln&&t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.method"),t.jsxs("select",{value:m,onChange:l=>c(l.target.value),children:[t.jsx("option",{value:"gauss",children:e("method.gauss")}),t.jsx("option",{value:"gauss-jordan",children:e("method.gaussJordan")})]})]}),mn&&t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.pivoting"),t.jsx("select",{value:o,onChange:l=>d(l.target.value),children:An.map(l=>t.jsx("option",{value:l,children:e(`pivot.${l}`)},l))})]})]}),K&&"note"in K&&K.note&&t.jsx("div",{className:"caption",children:a(K.note)}),s==="inverse"&&t.jsx("div",{className:"muted",style:{fontSize:"0.82rem"},children:e("lab.note.inverse")}),s==="tridiagonal"?t.jsxs("div",{className:"stack",children:[t.jsx(U,{label:i==="hu"?"Főátló d":"Diagonal d",values:u.d,horizontal:!0,onChange:(l,x)=>{p("custom"),v(f=>({...f,d:f.d.map((b,_)=>_===l?x:b)}))}}),t.jsx(U,{label:i==="hu"?"Felső átló c":"Super-diagonal c",values:u.c,horizontal:!0,onChange:(l,x)=>{p("custom"),v(f=>({...f,c:f.c.map((b,_)=>_===l?x:b)}))}}),t.jsx(U,{label:i==="hu"?"Alsó átló a":"Sub-diagonal a",values:u.a,horizontal:!0,onChange:(l,x)=>{p("custom"),v(f=>({...f,a:f.a.map((b,_)=>_===l?x:b)}))}}),t.jsx(U,{label:e("lab.vectorB"),values:u.b,horizontal:!0,onChange:(l,x)=>{p("custom"),v(f=>({...f,b:f.b.map((b,_)=>_===l?x:b)}))}})]}):t.jsxs("div",{className:"stack",children:[t.jsxs("label",{className:"field",children:[e("lab.size"),t.jsx("select",{value:_n,onChange:l=>vn(Number(l.target.value)),children:[2,3,4,5].map(l=>t.jsxs("option",{value:l,children:[l," × ",l]},l))})]}),t.jsx(xt,{label:e("lab.matrixA"),values:q,onChange:(l,x,f)=>{p("custom"),r(b=>b.map((_,H)=>H===l?_.map((V,yn)=>yn===x?f:V):_))}}),s==="solve"&&t.jsx(U,{label:e("lab.vectorB"),values:z,onChange:(l,x)=>{p("custom"),I(f=>f.map((b,_)=>_===l?x:b))}})]})]}),t.jsx("div",{className:"card stack",children:C.error||!N?t.jsx("p",{className:"feedback err",children:i==="hu"?"Érvénytelen bemenet.":"Invalid input."}):t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"caption",children:a(N.caption)}),t.jsx(gn,{matrix:N.matrix,coeffCols:s==="inverse"?void 0:N.coeffCols,pivot:N.pivot,changed:N.changed,varOrder:N.varOrder}),t.jsxs("div",{className:"kbd-controls",children:[t.jsxs("button",{className:"icon-btn",onClick:()=>y(0),disabled:O===0,children:["⏮ ",e("lab.first")]}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(l=>Math.max(0,l-1)),disabled:O===0,children:["◀ ",e("lab.prev")]}),t.jsx("button",{className:"btn",onClick:()=>Z(l=>!l),children:P?`⏸ ${e("lab.pause")}`:`▶ ${e("lab.play")}`}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(l=>Math.min(E.length-1,l+1)),disabled:O>=E.length-1,children:[e("lab.next")," ▶"]}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(E.length-1),disabled:O>=E.length-1,children:[e("lab.last")," ⏭"]})]}),t.jsxs("div",{className:"row",children:[t.jsx("span",{className:"muted",style:{fontSize:"0.85rem",minWidth:110},children:e("lab.stepOf",{a:O+1,b:E.length})}),t.jsx("div",{className:"progress",children:t.jsx("div",{style:{width:`${(O+1)/E.length*100}%`}})})]}),t.jsx(At,{run:C,mode:s})]})})]})}function At({run:n,mode:e}){const{t:a}=W();return n.singular&&e!=="determinant"?t.jsx("div",{className:"caption",children:a("lab.singular")}):t.jsxs("div",{className:"card",style:{background:"var(--bg-sunken)"},children:[t.jsx("div",{className:"section-eyebrow",children:a("lab.result")}),(e==="solve"||e==="tridiagonal")&&n.solution&&t.jsxs("div",{className:"result-line",style:{marginTop:8},children:[t.jsxs("strong",{children:[a("lab.solution"),":"]}),n.solution.map((i,s)=>t.jsx("span",{className:"pill",children:t.jsx(J,{tex:`x_{${s+1}} = ${an(i)}`})},s))]}),(e==="solve"||e==="determinant")&&n.determinant&&t.jsxs("div",{className:"result-line",style:{marginTop:8},children:[t.jsxs("strong",{children:[a("lab.determinant"),":"]}),t.jsx("span",{className:"pill",children:t.jsx(J,{tex:`\\det(A) = ${an(n.determinant)}`})})]}),e==="inverse"&&n.inverse&&t.jsxs("div",{style:{marginTop:8},children:[t.jsxs("strong",{children:[a("lab.inverse"),":"]}),t.jsx(gn,{matrix:n.inverse})]})]})}const Q=[{id:"q-tri-solve",kind:"vector",prompt:{en:"Solve this triangular system by backward substitution (enter x₁, x₂, x₃, x₄):",hu:"Oldd meg ezt a háromszög rendszert visszahelyettesítéssel (add meg x₁, x₂, x₃, x₄):"},tex:"\\begin{array}{rcrcrcrcr} 2x_1 &-& x_2 &+& 3x_3 &+& x_4 &=& 3\\\\ && 3x_2 &-& x_3 &+& 2x_4 &=& 13\\\\ &&&& 2x_3 &-& x_4 &=& -2\\\\ &&&&&& 3x_4 &=& 12 \\end{array}",answer:["-1","2","1","4"],solution:{en:"x₄ = 4, then x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1.",hu:"x₄ = 4, majd x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1."}},{id:"q-det-114",kind:"numeric",prompt:{en:"What is det(A) for the coefficient matrix of Example 3.39 (pivots 1, 3, 1, 38, no swaps)?",hu:"Mennyi det(A) a 3.39. példa együtthatómátrixára (főelemek 1, 3, 1, 38, csere nélkül)?"},answer:"114",solution:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"q-dd-invertible",kind:"truefalse",prompt:{en:"Every (row) diagonally dominant matrix is invertible.",hu:"Minden (soronként) diagonálisan domináns mátrix invertálható."},answer:!0,solution:{en:"True. Diagonal dominance implies Ax = 0 has only the trivial solution, so A is nonsingular.",hu:"Igaz. A diagonális dominanciából következik, hogy Ax = 0-nak csak triviális megoldása van, így A reguláris."}},{id:"q-partial-pivot",kind:"choice",prompt:{en:"In column 1 the entries (top to bottom) are 2, 2, −3, 2. Which row does partial pivoting move to the top?",hu:"Az 1. oszlop elemei (fentről le) 2, 2, −3, 2. Melyik sort viszi felülre a részleges főelemkiválasztás?"},choices:[{en:"Row 1 (value 2)",hu:"1. sor (érték 2)"},{en:"Row 3 (value −3)",hu:"3. sor (érték −3)"},{en:"The first nonzero row",hu:"Az első nemnulla sor"},{en:"No swap is needed",hu:"Nem kell csere"}],answer:1,solution:{en:"Partial pivoting picks the largest magnitude, |−3| = 3, so row 3 moves up.",hu:"A részleges főelemkiválasztás a legnagyobb abszolút értéket választja, |−3| = 3, így a 3. sor kerül felülre."}},{id:"q-gauss-cost",kind:"choice",prompt:{en:"What is the leading-order operation count of Gaussian elimination?",hu:"Mi a Gauss-elimináció vezető rendű műveletigénye?"},choices:[{en:"n²/2",hu:"n²/2"},{en:"n³/3",hu:"n³/3"},{en:"n³/2",hu:"n³/2"},{en:"5n − 4",hu:"5n − 4"}],answer:1,solution:{en:"Forward elimination plus back-substitution is n³/3 + O(n²) multiplications/divisions.",hu:"Az előre elimináció és a visszahelyettesítés együtt n³/3 + O(n²) szorzás/osztás."}},{id:"q-gj-cost",kind:"truefalse",prompt:{en:"Gauss–Jordan elimination is asymptotically cheaper than Gaussian elimination.",hu:"A Gauss–Jordan-elimináció aszimptotikusan olcsóbb, mint a Gauss-elimináció."},answer:!1,solution:{en:"False. Gauss–Jordan costs ≈ n³/2, more than Gaussian elimination’s n³/3.",hu:"Hamis. A Gauss–Jordan költsége ≈ n³/2, több, mint a Gauss-elimináció n³/3-a."}},{id:"q-thomas-cost",kind:"choice",prompt:{en:"How many multiplications/divisions does the Thomas (tridiagonal) algorithm need?",hu:"Hány szorzás/osztás kell a Thomas- (tridiagonális) algoritmushoz?"},choices:[{en:"n³/3",hu:"n³/3"},{en:"n²/2",hu:"n²/2"},{en:"5n − 4",hu:"5n − 4"},{en:"2ⁿ",hu:"2ⁿ"}],answer:2,solution:{en:"A tridiagonal solve is linear: 5n − 4 multiplications/divisions.",hu:"A tridiagonális megoldás lineáris: 5n − 4 szorzás/osztás."}},{id:"q-inv-entry",kind:"numeric",prompt:{en:"For A of Example 3.38, what is the (1,1) entry of A⁻¹? (Enter a fraction like -1/3.)",hu:"A 3.38. példa A mátrixára mi az A⁻¹ (1,1) eleme? (Adj meg törtet, pl. -1/3.)"},answer:"-1/3",solution:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], so the (1,1) entry is −1/3.",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], tehát az (1,1) elem −1/3."}},{id:"q-posdef",kind:"choice",prompt:{en:"A symmetric matrix is positive definite if and only if…",hu:"Egy szimmetrikus mátrix akkor és csak akkor pozitív definit, ha…"},choices:[{en:"all its entries are positive",hu:"minden eleme pozitív"},{en:"it is diagonally dominant",hu:"diagonálisan domináns"},{en:"all leading principal minors are positive",hu:"minden bal felső főminora pozitív"},{en:"its determinant is positive",hu:"a determinánsa pozitív"}],answer:2,solution:{en:"Sylvester’s criterion: positive definite ⇔ every leading principal minor is positive.",hu:"Sylvester-kritérium: pozitív definit ⇔ minden bal felső főminor pozitív."}}];function un(n,e){try{return Y(n.trim()).equals(Y(e.trim()))}catch{return!1}}function cn(n,e){switch(n.kind){case"choice":return e.choice===n.answer;case"truefalse":return e.bool===n.answer;case"numeric":return un(e.text??"",n.answer);case"vector":{const a=(e.text??"").split(/[,\s]+/).filter(Boolean);return a.length!==n.answer.length?!1:a.every((i,s)=>un(i,n.answer[s]))}}}function yt({item:n,onAnswered:e}){const{t:a,pick:i}=W(),[s,h]=A.useState({}),[m,c]=A.useState(!1),[o,d]=A.useState(!1),k=m&&cn(n,s);function g(){c(!0),e(cn(n,s))}return t.jsxs("div",{className:"card stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:a("quiz.title")}),t.jsx("p",{style:{marginBottom:6},children:i(n.prompt)}),n.tex&&t.jsx(J,{tex:n.tex,block:!0})]}),n.kind==="choice"&&t.jsx("div",{className:"choices",children:n.choices.map((p,T)=>{let q="choice";return s.choice===T&&(q+=" selected"),m&&T===n.answer&&(q+=" correct"),m&&s.choice===T&&T!==n.answer&&(q+=" wrong"),t.jsx("button",{className:q,onClick:()=>!m&&h({choice:T}),disabled:m,children:i(p)},T)})}),n.kind==="truefalse"&&t.jsxs("div",{className:"seg",children:[t.jsx("button",{className:s.bool===!0?"active":"",onClick:()=>!m&&h({bool:!0}),children:a("quiz.true")}),t.jsx("button",{className:s.bool===!1?"active":"",onClick:()=>!m&&h({bool:!1}),children:a("quiz.false")})]}),(n.kind==="numeric"||n.kind==="vector")&&t.jsx("input",{type:"text",placeholder:n.kind==="vector"?"x₁, x₂, …":a("quiz.placeholder"),value:s.text??"",onChange:p=>h({text:p.target.value}),disabled:m,style:{maxWidth:280}}),t.jsxs("div",{className:"row",children:[m?t.jsx("span",{className:k?"feedback ok":"feedback err",children:a(k?"quiz.correct":"quiz.incorrect")}):t.jsx("button",{className:"btn",onClick:g,children:a("quiz.check")}),t.jsx("button",{className:"btn secondary",onClick:()=>d(p=>!p),children:a(o?"quiz.hideSolution":"quiz.showSolution")})]}),o&&t.jsx("div",{className:"caption",children:i(n.solution)})]})}function wt(){const{t:n}=W(),[e,a]=A.useState(0),[i,s]=A.useState({}),h=Q[e],m=Object.values(i).filter(Boolean).length;return t.jsxs("div",{className:"stack",children:[t.jsxs("div",{className:"row",style:{justifyContent:"space-between"},children:[t.jsx("span",{className:"muted",children:n("quiz.questionOf",{a:e+1,b:Q.length})}),t.jsxs("span",{className:"pill",children:[n("quiz.score"),": ",m," / ",Object.keys(i).length]})]}),t.jsx(yt,{item:h,onAnswered:c=>s(o=>({...o,[e]:c}))},h.id),t.jsxs("div",{className:"row",children:[t.jsxs("button",{className:"icon-btn",onClick:()=>a(c=>Math.max(0,c-1)),disabled:e===0,children:["◀ ",n("quiz.prev")]}),t.jsxs("button",{className:"icon-btn",onClick:()=>a(c=>Math.min(Q.length-1,c+1)),disabled:e===Q.length-1,children:[n("quiz.next")," ▶"]})]})]})}const zt=`# Chapter 3: Exercise Solutions
...
- Back: xᵢ = (bᵢ - cᵢxᵢ₊₁)/dᵢ
`,jt=[...pn.map(n=>({id:n.id,no:n.number,title:n.title,blurb:n.summary})),{id:"lab",no:"3·lab",title:{en:"Elimination Lab",hu:"Eliminációs labor"},blurb:{en:"Step through Gaussian / Gauss–Jordan elimination interactively.",hu:"Lépkedj végig a Gauss- / Gauss–Jordan-eliminációi lépéseken."}},{id:"quiz",no:"3·quiz",title:{en:"Quiz",hu:"Kvíz"},blurb:{en:"Check your understanding.",hu:"Ellenőrizd a tudásod."}}];function Mt(){const{t:n,lang:e}=W(),a=zn();return A.useEffect(()=>{let i=decodeURIComponent(a.hash.replace(/^#/,""));if(!i){const s=a.pathname.match(/\/lessons\/([^/]+)/);s?i=s[1]:/\/lab$/.test(a.pathname)?i="lab":/\/quiz$/.test(a.pathname)&&(i="quiz")}i&&requestAnimationFrame(()=>{var s;return(s=document.getElementById(i))==null?void 0:s.scrollIntoView()})},[a.pathname,a.hash]),t.jsxs("div",{className:"app-shell ch-linear-systems",children:[t.jsx(qn,{sections:jt}),t.jsx("main",{children:t.jsxs("div",{className:"container",children:[t.jsxs("section",{className:"hero",id:"top",children:[t.jsx("span",{className:"section-eyebrow",children:n("app.subtitle")}),t.jsx("h1",{children:n("app.title")}),t.jsx("p",{children:n("home.tagline")}),t.jsx("p",{style:{marginTop:4},children:n("home.lead")})]}),pn.map(i=>t.jsxs("section",{id:i.id,className:"ls-section",children:[t.jsx(Nn,{section:i}),st(i.id).map(s=>t.jsx(In,{snippets:s.snippets,caption:s.caption},s.id))]},i.id)),t.jsxs("section",{id:"lab",className:"ls-section stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:n("nav.lab")}),t.jsx("h1",{style:{margin:"4px 0 2px"},children:n("nav.lab")})]}),t.jsx(_t,{initial:{}})]}),t.jsxs("section",{id:"quiz",className:"ls-section stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:n("nav.quiz")}),t.jsx("h1",{style:{margin:"4px 0 2px"},children:n("quiz.title")})]}),t.jsx(wt,{})]})]})}),t.jsx("footer",{className:"footer",children:t.jsx("div",{className:"container",children:e==="hu"?"Numerikus analízis · 3. fejezet — interaktív tananyag. F. Hartung nyomán, Pannon Egyetem.":"Numerical Analysis · Chapter 3 — interactive companion. After F. Hartung, University of Pannonia."})}),t.jsx(Tn,{markdown:zt})]})}export{Mt as default};
