...
`,xe=`// Gaussian elimination with complete (row + column) pivoting.
...
`,_e=`program gauss_elimination_demo
  implicit none
  integer, parameter :: n = 3
...
`,we=`function gauss_elimination(A, b)
    n = length(b)
    M = [A b]                       # augmented [A | b]
...
`,Ae=`function gaussElimination(A, b) {
  const n = b.length;
...
`,Pe=`function x = gauss_jordan(A, b)
...
`,De=`function gauss_partial_pivot(A, b)
...
`,Ve=`// Gaussian elimination with partial (row) pivoting.
