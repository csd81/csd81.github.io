...
`,Vn=`function back_substitution(U, b)
    n = length(b)
    x = zeros(n)
    for i in n:-1:1
...
`,Fn=`function backSubstitution(U, b) {
  const n = b.length;
...
`,Ee=`gaussPartial[Ain_, bin_] := Module[{A = N[Ain], b = N[bin], n = Length[bin], p, f, x},
   Do[
