...
`,Fe=`// Solve an upper-triangular system U x = b.
...
`,Ue=`backSubstitution[U_, b_] := Module[{n = Length[b], x},
   x = ConstantArray[0., n];
...
`,Xe=`program gauss_complete_demo
  implicit none
  integer, parameter :: n = 3
...
`,Ke=`function gauss_complete_pivot(A, b)
...
`,Qe=`// Gaussian elimination with complete (row + column) pivoting.
...
`,Ye=`function x = gauss_complete_pivot(A, b)
...
`,et=`// Gaussian elimination with complete (row + column) pivoting.
...
`,tt=`gaussComplete[Ain_, bin_] := Module[
...
`,it=`program gauss_elimination_demo
  implicit none
  integer, parameter :: n = 3
...
`,st=`function gauss_elimination(A, b)
    n = length(b)
    M = [A b]                       # augmented [A | b]
...
`,ot=`function gaussElimination(A, b) {
  const n = b.length;
...
`,rt=`function x = gauss_elimination(A, b)
...
`,mt=`// Naive Gaussian elimination (no pivoting) + back-substitution.
...
`,ht=`gaussElimination[A_, b_] := Module[{n = Length[b], M, x, f},
...
`,bt=`// Solve A x = b by Gauss-Jordan elimination.
...
`,pt=`function x = gauss_jordan(A, b)
...
`,gt=`// Solve A x = b by Gauss-Jordan elimination (reduced row echelon form).
...
`,kt=`gaussJordan[Ain_, bin_] := Module[{M = N@MapThread[Append, {Ain, bin}], n = Length[bin], p, f},
   Do[
...
`,At=`function gauss_partial_pivot(A, b)
...
`,yt=`// Gaussian elimination with partial (row) pivoting.
...
`,wt=`function x = gauss_partial_pivot(A, b)
...
`,jt=`// Gaussian elimination with partial (row) pivoting.
...
`,qt=`gaussPartial[Ain_, bin_] := Module[{A = N[Ain], b = N[bin], n = Length[bin], p, f, x},
   Do[
...
`,Tt=`program matrix_inverse_demo
  implicit none
  integer, parameter :: n = 3
...
println(inverse(A))
`,Wt=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
function inverse(A) {
  const n = A.length;
...
`,St=`function Ainv = matrix_inverse(A)
...
`,Bt=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
...
`,Nt=`inverse[Ain_] := Module[{A = N[Ain], n = Length[Ain], M, p, d},
...
`,Vt=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
`,Ft=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
`,Ut=`thomas[a_, b_, cIn_, dIn_] := Module[{n = Length[dIn], c = cIn, d = dIn, x, m},
   c[[1]] /= b[[1]]; d[[1]] /= b[[1]];
   Do[m = b[[i]] - a[[i]] c[[i - 1]];
      If[i < n, c[[i]] /= m];
