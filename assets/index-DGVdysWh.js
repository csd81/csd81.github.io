...
`,Tn={lagrange:{en:kn,hu:bn},newton:{en:vn,hu:zn},hermite:{en:wn,hu:jn},spline:{en:qn,hu:Ln}};function Sn(t,a){var s;return((s=Tn[t])==null?void 0:s[a])??""}const An=`#include <vector>
#include <iostream>
using namespace std;
...
`,Nn=`function hermite_coeffs(x, y, dy)
...
println(a)   # [1, 0, -1, 2]
`,Cn=`// Hermite interpolation via divided differences with doubled nodes.
...
`,In=`function [z, a] = hermite_coeffs(x, y, dy)
...
`,Mn=`# Hermite interpolation via divided differences with doubled nodes.
...
`,Pn=`// Hermite interpolation via divided differences with doubled nodes.
...
`,Vn=`hermiteCoeffs[x_, y_, dy_] := Module[{n = Length[x], m, z, Q, i, j},
...
`,Rn=`function lagrange_coeffs(x, y)
    n = length(x)
...
`,Bn=`// Lagrange interpolation via the Vandermonde system V a = y.
function solve(A, b) {
  const n = b.length;
...
`,On=`function a = lagrange_coeffs(x, y)
...
`,Gn=`# Polynomial coefficients a (low->high) of the degree n-1 interpolant
# through (x_i, y_i), via the Vandermonde system V a = y.
...
`,Kn=`// Lagrange interpolation via the Vandermonde system V a = y.
...
`,Xn=`lagrangeCoeffs[x_, y_] := Module[{n = Length[x], V},
...
`,ee=`// Evaluate the Newton form by nested (Horner-like) multiplication.
...
`,te=`function p = newton_eval(x, a, t)
...
`,ae=`# Evaluate the Newton form  a_0 + a_1(t-x_0) + a_2(t-x_0)(t-x_1) + ...
# by nested (Horner-like) multiplication.
...
`,oe=`// Evaluate the Newton form by nested (Horner-like) multiplication.
...
`,se=`newtonEval[x_, a_, t_] := Module[{p = Last[a], k},
...
`,re=`program divided_differences_demo
  implicit none
  integer, parameter :: n = 4
  real(8) :: x(n), a(n)
  integer :: i, j
...
`,_e=`function divided_differences(x, y)
...
`,$e=`// Newton divided-difference coefficients a_i = f[x_0, ..., x_i].
...
`,de=`function a = divided_differences(x, y)
...
`,ce=`# Newton divided differences and Horner evaluation of the Newton form.
...
`,he=`// Newton divided-difference coefficients a_i = f[x_0, ..., x_i].
...
`,me=`dividedDifferences[x_, y_] := Module[{a = N[y], n = Length[x], i, j},
...
`,ye=`function natural_cubic_spline(x, y)
    n = length(x); h = diff(x)
...
println("a = ", a)
`,ke=`// Natural cubic spline: per-interval coefficients (a, b, c, d).
function solve(A, b) {
  const n = b.length;
  const m = A.map((r) => [...r]), r = [...b];
  for (let k = 0; k < n; k++) {
...
`,be=`function [a, b, c, d] = natural_cubic_spline(x, y)
...
if __name__ == "__main__":
    ...
    print("c =", c)
    print("d =", d)
`,ze=`# Natural cubic spline. Returns per-interval (a, b, c, d) with
# S_i(t) = a_i + b_i (t - x_i) + c_i (t - x_i)^2 + d_i (t - x_i)^3.
...
cat("c =", res$c, "\\n")
cat("d =", res$d, "\\n")
`,we=`// Natural cubic spline: per-interval coefficients (a, b, c, d).
fn solve(mut a: Vec<Vec<f64>>, mut b: Vec<f64>) -> Vec<f64> {
    let n = b.len();
    for k in 0..n {
        let mut p = k;
...
`,je=`naturalCubicSpline[x_, y_] := Module[{n = Length[x], h, A, rhs, c, a, b, d},
   h = Differences[x];
...
`,qe=Object.assign({"./hermite.cpp":An,"./hermite.f90":Hn,"./hermite.go":Dn,"./hermite.jl":Nn,"./hermite.js":Cn,"./hermite.m":In,"./hermite.py":En,"./hermite.r":Mn,"./hermite.rs":Pn,"./hermite.wl":Vn,"./lagrange.cpp":Wn,"./lagrange.f90":Qn,"./lagrange.go":Fn,"./lagrange.jl":Rn,"./lagrange.js":Bn,"./lagrange.m":On,"./lagrange.py":Un,"./lagrange.r":Gn,"./lagrange.rs":Kn,"./lagrange.wl":Xn,"./newton-eval.cpp":Yn,"./newton-eval.f90":Jn,"./newton-eval.go":Zn,"./newton-eval.jl":ne,"./newton-eval.js":ee,"./newton-eval.m":te,"./newton-eval.py":ie,"./newton-eval.r":ae,"./newton-eval.rs":oe,"./newton-eval.wl":se,"./newton.cpp":le,"./newton.f90":re,"./newton.go":xe,"./newton.jl":_e,"./newton.js":$e,"./newton.m":de,"./newton.py":fe,"./newton.r":ce,"./newton.rs":he,"./newton.wl":me,"./spline.cpp":pe,"./spline.f90":ge,"./spline.go":ue,"./spline.jl":ye,"./spline.js":ke,"./spline.m":be,"./spline.py":ve,"./spline.r":ze,"./spline.rs":we,"./spline.wl":je}),j=(t,a)=>qe[`./${t}.${a}`],Le={lagrange:{en:"Lagrange interpolation (Vandermonde system)",hu:"Lagrange-interpoláció (Vandermonde-rendszer)"},newton:{en:"Newton's divided differences (coefficients)",hu:"Newton-féle osztott differenciák (együtthatók)"},"newton-eval":{en:"Newton polynomial evaluation (nested form)",hu:"Newton-polinom kiértékelése (beágyazott alak)"},hermite:{en:"Hermite interpolation",hu:"Hermite-interpoláció"},spline:{en:"Natural cubic spline",hu:"Természetes köbös spline"}},Te=t=>({id:t,caption:Le[t],snippets:{matlab:j(t,"m"),python:j(t,"py"),cpp:j(t,"cpp"),julia:j(t,"jl"),rust:j(t,"rs"),fortran:j(t,"f90"),wolfram:j(t,"wl"),javascript:j(t,"js"),go:j(t,"go"),r:j(t,"r")}}),Se={lagrange:["lagrange"],newton:["newton","newton-eval"],hermite:["hermite"],spline:["spline"]};function Ae(t){return(Se[t]??[]).map(Te)}function E({str:t,method:a,points:s,derivatives:i,allowCompare:r,showTable:o,enableDerivatives:l}){const{lang:_}=W(),e=t.lessons[a],$=Sn(a,_),m=Ae(a);return n.jsxs("article",{className:"lesson",children:[n.jsxs("header",{children:[n.jsx("h2",{children:e.title}),n.jsx("p",{className:"intro",children:e.intro})]}),n.jsxs("section",{className:"theorem-card",children:[n.jsx("div",{className:"theorem-tag",children:e.theoremTitle}),n.jsx(nn,{block:!0,children:e.theorem}),n.jsx("p",{children:e.body})]}),$&&n.jsxs("details",{className:"lesson__theory",open:!0,children:[n.jsx("summary",{children:_==="hu"?"Elmélet":"Theory"}),n.jsx(V,{markdown:$})]}),m.map(c=>n.jsx(Y,{snippets:c.snippets,caption:c.caption},c.id)),n.jsxs("p",{className:"tryit",children:["👉 ",e.tryIt]}),n.jsx(hn,{str:t,initialPoints:s,initialDerivs:i,primary:a,allowCompare:r,showTable:o,enableDerivatives:l}),n.jsx(gn,{deck:a}),n.jsx(yn,{deck:a})]})}const M={lagrange:"#ff5d8f",newton:"#f5a623",hermite:"#9b5de5",spline:"#00bbf9"},U=t=>Math.cos(t),P=[-Math.PI-.5,Math.PI+.5],He=[-2.5,2.5];function De(){const t=Array.from({length:40}),a=["#ff5d8f","#f5a623","#9b5de5","#00bbf9","#3ddc97"];return n.jsx("div",{className:"confetti","aria-hidden":!0,children:t.map((s,i)=>n.jsx("span",{style:{left:`${Math.random()*100}%`,background:a[i%a.length],animationDelay:`${Math.random()*.6}s`,transform:`rotate(${Math.random()*360}deg)`}},i))})}function Ne({str:t}){const[a,s]=p.useState([{x:-3,y:-.5},{x:-1.2,y:.5},{x:.5,y:.7},{x:2,y:-.2},{x:3,y:-.8}]),[i,r]=p.useState("spline"),[o,l]=p.useState(!0),_=p.useMemo(()=>[...a].sort((h,u)=>h.x-u.x),[a]),e=p.useMemo(()=>Q(i,_),[i,_]),$=p.useMemo(()=>{let h=0;for(let u=0;u<=80;u++){const b=P[0]+(P[1]-P[0])*u/80,L=Math.abs(e(b)-U(b));Number.isFinite(L)&&(h=Math.max(h,L))}return h},[e]),m=$<.18,c=[{fn:e,color:M[i],label:t.methods[i]}];o&&c.push({fn:U,color:"#8aa0b5",label:"cos x",dashed:!0});const g=(h,u,b)=>s(L=>L.map((S,A)=>A===h?{x:u,y:b}:S));return n.jsxs("div",{className:"challenge",children:[n.jsxs("div",{className:"challenge-head",children:[n.jsx("h2",{children:t.nav.playground}),n.jsx("p",{className:"intro",children:t.tagline})]}),n.jsxs("div",{className:"challenge-bar",children:[n.jsx("div",{className:"methodpills",children:Object.keys(M).map(h=>n.jsx("button",{className:`pill ${i===h?"on":""}`,style:i===h?{background:M[h],borderColor:M[h]}:{},onClick:()=>r(h),children:t.methods[h]},h))}),n.jsxs("label",{className:"chip",children:[n.jsx("input",{type:"checkbox",checked:o,onChange:()=>l(h=>!h)}),t.ui.showCos]})]}),n.jsxs("div",{className:"challenge-plotwrap",children:[n.jsx(G,{points:a,curves:c,domain:P,range:He,onDrag:g,width:720,height:440}),m&&n.jsx(De,{})]}),n.jsx("div",{className:`challenge-status ${m?"win":""}`,children:m?n.jsx("strong",{children:t.ui.nailedIt}):n.jsxs("span",{children:["🎯 ",t.ui.challengeHint," ",n.jsxs("em",{children:["(max Δ = ",$.toFixed(2),")"]})]})})]})}const Ce=`# Chapter 6: Exercise Solutions
...
- Lagrange: O(hⁿ⁺¹)
- Cubic spline: O(h⁴)
`,Ie=[{id:"play",no:"6·play",title:{en:w.en.nav.playground,hu:w.hu.nav.playground},blurb:{en:"",hu:""}},{id:"lagrange",no:"6.1",title:{en:w.en.nav.lagrange,hu:w.hu.nav.lagrange},blurb:{en:"",hu:""}},{id:"newton",no:"6.2",title:{en:w.en.nav.newton,hu:w.hu.nav.newton},blurb:{en:"",hu:""}},{id:"hermite",no:"6.4",title:{en:w.en.nav.hermite,hu:w.hu.nav.hermite},blurb:{en:"",hu:""}},{id:"spline",no:"6.5",title:{en:w.en.nav.spline,hu:w.hu.nav.spline},blurb:{en:"",hu:""}}];function Pe(){const{lang:t}=W(),a=w[t],s={scrollMarginTop:"calc(var(--nav-h) + var(--scrolly-topbar-h, 44px) + 8px)"};return n.jsxs("div",{className:"app ch-interpolation",children:[n.jsx(J,{sections:Ie}),n.jsx("header",{className:"topbar",children:n.jsxs("div",{className:"brand",children:[n.jsx("span",{className:"logo",children:"📈"}),n.jsxs("div",{children:[n.jsx("div",{className:"brand-title",children:a.appTitle}),n.jsx("div",{className:"brand-sub",children:a.tagline})]})]})}),n.jsxs("main",{className:"content",children:[n.jsx("section",{id:"play",style:s,children:n.jsx(Ne,{str:a})}),n.jsx("section",{id:"lagrange",style:s,children:n.jsx(E,{str:a,method:"lagrange",points:[{x:-1,y:-3},{x:1,y:1},{x:2,y:3},{x:3,y:29}],allowCompare:!0})}),n.jsx("section",{id:"newton",style:s,children:n.jsx(E,{str:a,method:"newton",points:[{x:-1,y:-2},{x:1,y:0},{x:2,y:-2},{x:3,y:2}],showTable:!0,allowCompare:!0})}),n.jsx("section",{id:"hermite",style:s,children:n.jsx(E,{str:a,method:"hermite",points:[{x:-1,y:2},{x:1,y:4},{x:2,y:11}],derivatives:[3,-5,30],enableDerivatives:!0})}),n.jsx("section",{id:"spline",style:s,children:n.jsx(E,{str:a,method:"spline",points:[{x:0,y:.5},{x:1,y:.1},{x:1.5,y:2.5},{x:2,y:-1},{x:3,y:-.5},{x:4,y:0}],allowCompare:!0})})]}),n.jsx("footer",{className:"foot",children:"InterPlay · Numerical Analysis 6 · Interpolation — F. Hartung, University of Pannonia"}),n.jsx(Z,{markdown:Ce})]})}export{Pe as default};
