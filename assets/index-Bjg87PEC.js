...
- Back: xᵢ = (bᵢ - cᵢxᵢ₊₁)/dᵢ
`;function Jn(n){const e=new Map,o=n.split(`
`);let i=null,s=null;const h=()=>{if(i&&s){const l=e.get(i)??[];s.body=s.body.replace(/\n+$/,""),l.push(s),e.set(i,l)}s=null};for(const l of o){const d=l.match(/^##\s+Section\s+([\d.]+)\s+Exercises/i);if(d){h(),i=d[1];continue}if(/^##\s+/.test(l)&&!d){h(),i=null;continue}const a=l.match(/^###\s+Exercise\s+(\d+)\s*:?\s*(.*)$/i);if(a&&i){h(),s={n:a[1],title:a[2].trim(),body:""};continue}if(s){if(l.trim()==="---")continue;s.body+=l+`
`}}return h(),e}function Cn({sectionNumber:n}){const{t:e}=S(),i=A.useMemo(()=>Jn(Nn),[]).get(n);if(!i||i.length===0)return null;const s=e("common.showSolution");return t.jsxs("section",{className:"ls-exercises card stack","aria-label":"exercises",children:[t.jsxs("span",{className:"section-eyebrow",children:[e("common.exercises")," · §",n]}),i.map(h=>{const l=`**${e("common.exercise")} ${h.n}${h.title?`: ${h.title}`:""}**

<details class="reveal-solution"><summary>${s}</summary>

${h.body}

</details>`;return t.jsx(sn,{markdown:l},h.n)})]})}const Ln=`#include <vector>
#include <iostream>
using namespace std;
...
`,Vn=`program back_substitution_demo
  implicit none
...
`,Dn=`function back_substitution(U, b)
    n = length(b)
    x = zeros(n)
    for i in n:-1:1
...
`,Hn=`function backSubstitution(U, b) {
  const n = b.length;
...
`,Un=`function x = back_substitution(U, b)
...
`,Xn=`back_substitution <- function(U, b) {
  # Solve an upper-triangular system U x = b.
  n <- length(b)
  x <- numeric(n)
  for (i in n:1) {
...
b <- c(1, 8, 9)
print(back_substitution(U, b))    # [1, 2, 3]
`,Kn=`// Solve an upper-triangular system U x = b.
...
`,Qn=`backSubstitution[U_, b_] := Module[{n = Length[b], x},
   x = ConstantArray[0., n];
...
`,Zn=`program forward_substitution_demo
  implicit none
  integer, parameter :: n = 3
...
`,ee=`function forward_substitution(L, b)
    n = length(b)
    y = zeros(n)
    for i in 1:n
...
`,te=`function forwardSubstitution(L, b) {
  const n = b.length;
...
`,ie=`function y = forward_substitution(L, b)
...
`,ae=`def forward_substitution(L, b):
    """Solve a lower-triangular system L y = b."""
    n = len(b)
    y = [0.0] * n
    for i in range(n):
...
`,oe=`forward_substitution <- function(L, b) {
  # Solve a lower-triangular system L y = b.
  n <- length(b)
  y <- numeric(n)
  for (i in 1:n) {
...
b <- c(4, 5, -1)
print(forward_substitution(L, b))    # [2, 1, 0]
`,se=`// Solve a lower-triangular system L y = b.
...
`,re=`forwardSubstitution[L_, b_] := Module[{n = Length[b], y},
   y = ConstantArray[0., n];
...
`,me=`program gauss_complete_demo
  implicit none
  integer, parameter :: n = 3
...
`,de=`function gauss_complete_pivot(A, b)
...
`,ue=`// Gaussian elimination with complete (row + column) pivoting.
...
`,ce=`function x = gauss_complete_pivot(A, b)
...
`,be=`gauss_complete_pivot <- function(A, b) {
  # Gaussian elimination with complete (row+column) pivoting.
  # Returns x (column swaps are undone before returning).
...
`,ge=`gaussComplete[Ain_, bin_] := Module[
...
`,xe=`program gauss_elimination_demo
  implicit none
  integer, parameter :: n = 3
...
`,ve=`function gauss_elimination(A, b)
    n = length(b)
    M = [A b]                       # augmented [A | b]
...
`,_e=`function gaussElimination(A, b) {
  const n = b.length;
...
`,Ae=`function x = gauss_elimination(A, b)
...
`,we=`gauss_elimination <- function(A, b) {
  # Naive Gaussian elimination (no pivoting) + back-substitution.
  n <- length(b)
...
b <- c(8, -11, -3)
print(gauss_elimination(A, b))    # [2, 3, -1]
`,je=`// Naive Gaussian elimination (no pivoting) + back-substitution.
...
`,ze=`gaussElimination[A_, b_] := Module[{n = Length[b], M, x, f},
...
`,Ge=`// Solve A x = b by Gauss-Jordan elimination.
...
`,Se=`function x = gauss_jordan(A, b)
...
`,Pe=`// Solve A x = b by Gauss-Jordan elimination (reduced row echelon form).
...
`,Be=`gaussJordan[Ain_, bin_] := Module[{M = N@MapThread[Append, {Ain, bin}], n = Length[bin], p, f},
   Do[
...
`,Ce=`function gauss_partial_pivot(A, b)
...
`,Le=`// Gaussian elimination with partial (row) pivoting.
...
`,Ve=`function x = gauss_partial_pivot(A, b)
...
`,De=`gauss_partial_pivot <- function(A, b) {
  # Solve A x = b by Gaussian elimination with partial (row) pivoting.
  A <- matrix(as.numeric(A), nrow = nrow(A))
  b <- as.numeric(b)
  n <- length(b)
...
`,He=`// Gaussian elimination with partial (row) pivoting.
...
`,Ue=`gaussPartial[Ain_, bin_] := Module[{A = N[Ain], b = N[bin], n = Length[bin], p, f, x},
   Do[
...
`,Xe=`program matrix_inverse_demo
  implicit none
  integer, parameter :: n = 3
...
println(inverse(A))
`,Ye=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
function inverse(A) {
  const n = A.length;
...
`,Ze=`function Ainv = matrix_inverse(A)
...
`,tt=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
...
`,it=`inverse[Ain_] := Module[{A = N[Ain], n = Length[Ain], M, p, d},
...
`,lt=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
`,ct=`thomas[a_, b_, cIn_, dIn_] := Module[{n = Length[dIn], c = cIn, d = dIn, x, m},
   c[[1]] /= b[[1]]; d[[1]] /= b[[1]];
   Do[m = b[[i]] - a[[i]] c[[i - 1]];
      If[i < n, c[[i]] /= m];
...
`,ft=Object.assign({"./back-substitution.cpp":Ln,"./back-substitution.f90":Vn,"./back-substitution.go":Fn,"./back-substitution.jl":Dn,"./back-substitution.js":Hn,"./back-substitution.m":Un,"./back-substitution.py":Rn,"./back-substitution.r":Xn,"./back-substitution.rs":Kn,"./back-substitution.wl":Qn,"./forward-substitution.cpp":Yn,"./forward-substitution.f90":Zn,"./forward-substitution.go":ne,"./forward-substitution.jl":ee,"./forward-substitution.js":te,"./forward-substitution.m":ie,"./forward-substitution.py":ae,"./forward-substitution.r":oe,"./forward-substitution.rs":se,"./forward-substitution.wl":re,"./gauss-complete.cpp":le,"./gauss-complete.f90":me,"./gauss-complete.go":he,"./gauss-complete.jl":de,"./gauss-complete.js":ue,"./gauss-complete.m":ce,"./gauss-complete.py":fe,"./gauss-complete.r":be,"./gauss-complete.rs":pe,"./gauss-complete.wl":ge,"./gauss-elimination.cpp":$e,"./gauss-elimination.f90":xe,"./gauss-elimination.go":ke,"./gauss-elimination.jl":ve,"./gauss-elimination.js":_e,"./gauss-elimination.m":Ae,"./gauss-elimination.py":ye,"./gauss-elimination.r":we,"./gauss-elimination.rs":je,"./gauss-elimination.wl":ze,"./gauss-jordan.cpp":qe,"./gauss-jordan.f90":Te,"./gauss-jordan.go":Ie,"./gauss-jordan.jl":Me,"./gauss-jordan.js":Ge,"./gauss-jordan.m":Se,"./gauss-jordan.py":We,"./gauss-jordan.r":Ee,"./gauss-jordan.rs":Pe,"./gauss-jordan.wl":Be,"./gauss-partial.cpp":Oe,"./gauss-partial.f90":Ne,"./gauss-partial.go":Je,"./gauss-partial.jl":Ce,"./gauss-partial.js":Le,"./gauss-partial.m":Ve,"./gauss-partial.py":Fe,"./gauss-partial.r":De,"./gauss-partial.rs":He,"./gauss-partial.wl":Ue,"./matrix-inverse.cpp":Re,"./matrix-inverse.f90":Xe,"./matrix-inverse.go":Ke,"./matrix-inverse.jl":Qe,"./matrix-inverse.js":Ye,"./matrix-inverse.m":Ze,"./matrix-inverse.py":nt,"./matrix-inverse.r":et,"./matrix-inverse.rs":tt,"./matrix-inverse.wl":it,"./tridiagonal.cpp":at,"./tridiagonal.f90":ot,"./tridiagonal.go":st,"./tridiagonal.jl":rt,"./tridiagonal.js":lt,"./tridiagonal.m":mt,"./tridiagonal.py":ht,"./tridiagonal.r":dt,"./tridiagonal.rs":ut,"./tridiagonal.wl":ct}),M=(n,e)=>ft[`./${n}.${e}`],bt={"forward-substitution":{en:"Forward substitution (lower-triangular system)",hu:"Előrehelyettesítés (alsó háromszögű rendszer)"},"back-substitution":{en:"Back-substitution (upper-triangular system)",hu:"Visszahelyettesítés (felső háromszögű rendszer)"},"gauss-elimination":{en:"Gaussian elimination (no pivoting)",hu:"Gauss-elimináció (főelemkiválasztás nélkül)"},"gauss-partial":{en:"Gaussian elimination — partial pivoting",hu:"Gauss-elimináció — részleges főelemkiválasztás"},"gauss-complete":{en:"Gaussian elimination — complete pivoting",hu:"Gauss-elimináció — teljes főelemkiválasztás"},"gauss-jordan":{en:"Gauss–Jordan elimination",hu:"Gauss–Jordan-elimináció"},"matrix-inverse":{en:"Matrix inverse (Gauss–Jordan on [A | I])",hu:"Mátrixinverz (Gauss–Jordan az [A | I] mátrixon)"},tridiagonal:{en:"Tridiagonal solver (Thomas algorithm)",hu:"Tridiagonális megoldó (Thomas-algoritmus)"}},pt=n=>({id:n,caption:bt[n],snippets:{matlab:M(n,"m"),python:M(n,"py"),cpp:M(n,"cpp"),julia:M(n,"jl"),rust:M(n,"rs"),fortran:M(n,"f90"),wolfram:M(n,"wl"),javascript:M(n,"js"),go:M(n,"go"),r:M(n,"r")}}),gt={s33:["forward-substitution","back-substitution","gauss-elimination","gauss-partial","gauss-complete","gauss-jordan","matrix-inverse","tridiagonal"]};function $t(n){return(gt[n]??[]).map(pt)}function Y(n,e){return new rn(n)}const R=new rn(0),pn=new rn(1);function W(n){return n.n===0}function an(n){if(n.n===0)return"0";const e=n.s<0?"-":"";return n.d===1?`${e}${n.n}`:`${e}\\frac{${n.n}}{${n.d}}`}function z(n){if(n.n===0)return"0";const e=n.s<0?"-":"";return n.d===1?`${e}${n.n}`:`${e}${n.n}/${n.d}`}function X(n){return n.map(e=>e.slice())}function xt(n){const e=[];for(let o=0;o<n;o++)e.push(Array.from({length:n},(i,s)=>Y(o===s?1:0)));return e}function kt(n,e){return n.map((o,i)=>o.concat(e[i]))}const vt=n=>n.length?n[0].length:0;function _t(n,e,o){return n.map(i=>i.slice(e,o))}function g(n){return n+1}function ln(n,e,o){const i=X(n),s=i.length,h=vt(i),l=Array.from({length:e},(r,j)=>j),d=[],a=[];let u=0,k=0,$=!1;const b=()=>X(i);a.push({kind:"init",matrix:b(),coeffCols:e,varOrder:l.slice(),caption:{en:"Initial augmented matrix.",hu:"Kiindulási kibővített mátrix."}});const I=r=>{let j=R;for(let T=0;T<e;T++){const c=i[r][T].abs();c.compare(j)>0&&(j=c)}return j},q=Math.min(s,e);for(let r=0;r<q;r++){if(o.pivoting==="partial"){let c=r;for(let v=r+1;v<s;v++)i[v][r].abs().compare(i[c][r].abs())>0&&(c=v);!W(i[c][r])&&c!==r&&([i[r],i[c]]=[i[c],i[r]],u++,a.push({kind:"row-swap",matrix:b(),coeffCols:e,varOrder:l.slice(),swapRows:[r,c],caption:{en:`Partial pivoting: swap rows ${g(r)} and ${g(c)} (largest |entry| in column ${g(r)}).`,hu:`Részleges főelemkiválasztás: ${g(r)}. és ${g(c)}. sor cseréje (legnagyobb |elem| a(z) ${g(r)}. oszlopban).`}}))}else if(o.pivoting==="scaled"){let c=r,v=null;for(let w=r;w<s;w++){const y=I(w),E=W(y)?R:i[w][r].abs().div(y);(v===null||E.compare(v)>0)&&(v=E,c=w)}!W(i[c][r])&&c!==r&&([i[r],i[c]]=[i[c],i[r]],u++,a.push({kind:"row-swap",matrix:b(),coeffCols:e,varOrder:l.slice(),swapRows:[r,c],caption:{en:`Scaled partial pivoting: swap rows ${g(r)} and ${g(c)} (largest scaled ratio).`,hu:`Skálázott főelemkiválasztás: ${g(r)}. és ${g(c)}. sor cseréje (legnagyobb skálázott hányados).`}}))}else if(o.pivoting==="complete"){let c=r,v=r;for(let w=r;w<s;w++)for(let y=r;y<e;y++)i[w][y].abs().compare(i[c][v].abs())>0&&(c=w,v=y);if(!W(i[c][v])&&(c!==r&&([i[r],i[c]]=[i[c],i[r]],u++,a.push({kind:"row-swap",matrix:b(),coeffCols:e,varOrder:l.slice(),swapRows:[r,c],caption:{en:`Complete pivoting: swap rows ${g(r)} and ${g(c)}.`,hu:`Teljes főelemkiválasztás: ${g(r)}. és ${g(c)}. sor cseréje.`}})),v!==r)){for(let w=0;w<s;w++)[i[w][r],i[w][v]]=[i[w][v],i[w][r]];[l[r],l[v]]=[l[v],l[r]],k++,a.push({kind:"col-swap",matrix:b(),coeffCols:e,varOrder:l.slice(),swapCols:[r,v],caption:{en:`Complete pivoting: swap columns ${g(r)} and ${g(v)} (track variable order).`,hu:`Teljes főelemkiválasztás: ${g(r)}. és ${g(v)}. oszlop cseréje (a változók sorrendjét követjük).`}})}}const j=i[r][r];if(d.push(j),W(j)){$=!0,a.push({kind:"pivot-select",matrix:b(),coeffCols:e,varOrder:l.slice(),pivot:[r,r],caption:{en:`Pivot a_{${g(r)}${g(r)}} = 0 and no nonzero entry is available — the elimination stops (matrix is singular${o.pivoting==="none"?" for this strategy":""}).`,hu:`A főelem a_{${g(r)}${g(r)}} = 0, és nincs nemnulla elem — az elimináció megáll (a mátrix szinguláris${o.pivoting==="none"?" ennél a stratégiánál":""}).`}});break}a.push({kind:"pivot-select",matrix:b(),coeffCols:e,varOrder:l.slice(),pivot:[r,r],caption:{en:`Pivot a_{${g(r)}${g(r)}} = ${z(j)}.`,hu:`Főelem a_{${g(r)}${g(r)}} = ${z(j)}.`}});const T=o.method==="gauss-jordan"?0:r+1;for(let c=T;c<s;c++){if(c===r||W(i[c][r]))continue;const v=i[c][r].div(j),w=[];for(let y=r;y<h;y++){const E=i[c][y];i[c][y]=i[c][y].sub(v.mul(i[r][y])),i[c][y].equals(E)||w.push([c,y])}a.push({kind:"eliminate",matrix:b(),coeffCols:e,varOrder:l.slice(),pivot:[r,r],multiplier:v,changed:w,caption:{en:`Eliminate: R${g(c)} ← R${g(c)} − (${z(v)})·R${g(r)}.`,hu:`Kiküszöbölés: S${g(c)} ← S${g(c)} − (${z(v)})·S${g(r)}.`}})}}if(o.method==="gauss-jordan"&&!$)for(let r=0;r<q;r++){const j=i[r][r];if(W(j)||j.equals(pn))continue;const T=[];for(let c=0;c<h;c++)i[r][c]=i[r][c].div(j),T.push([r,c]);a.push({kind:"normalize",matrix:b(),coeffCols:e,varOrder:l.slice(),pivot:[r,r],changed:T,caption:{en:`Normalise: R${g(r)} ← R${g(r)} / (${z(j)}).`,hu:`Normálás: S${g(r)} ← S${g(r)} / (${z(j)}).`}})}return{steps:a,matrix:i,coeffCols:e,varOrder:l,pivots:d,rowSwaps:u,colSwaps:k,singular:$}}function gn(n){if(n.coeffCols!==n.matrix.length)return null;if(n.singular)return R;let e=pn;for(const o of n.pivots)e=e.mul(o);return(n.rowSwaps+n.colSwaps)%2===1&&(e=e.neg()),e}function At(n){const{matrix:e,coeffCols:o,varOrder:i,singular:s}=n;if(s)return{steps:[],solution:null};const h=o,l=new Array(o),d=[];for(let u=o-1;u>=0;u--){let k=e[u][h];for(let b=u+1;b<o;b++)k=k.sub(e[u][b].mul(l[b]));l[u]=k.div(e[u][u]);const $=i[u]+1;d.push({kind:"back-sub",matrix:X(e),coeffCols:o,varOrder:i.slice(),pivot:[u,u],caption:{en:`Back-substitution: x${$} = ${z(l[u])}.`,hu:`Visszahelyettesítés: x${$} = ${z(l[u])}.`}})}const a=new Array(o);for(let u=0;u<o;u++)a[i[u]]=l[u];return{steps:d,solution:a}}function yt(n,e){return n.map((o,i)=>o.concat([e[i]]))}function wt(n,e,o){const i=n[0].length,s=yt(n,e),h=ln(s,i,o),l=gn(h);if(h.singular)return{steps:h.steps,solution:null,singular:!0,determinant:l};let d=h.steps,a;if(o.method==="gauss"){const k=At(h);d=d.concat(k.steps),a=k.solution}else{const k=i;a=new Array(i);for(let $=0;$<i;$++)a[h.varOrder[$]]=h.matrix[$][k]}const u=a.map((k,$)=>`x${$+1} = ${z(k)}`).join(",  ");return d=d.concat([{kind:"done",matrix:X(h.matrix),coeffCols:i,varOrder:h.varOrder.slice(),caption:{en:`Solution: ${u}.`,hu:`Megoldás: ${u}.`}}]),{steps:d,solution:a,singular:!1,determinant:l}}function jt(n,e="none"){const o=n.length,i=kt(n,xt(o)),s=ln(i,o,{method:"gauss-jordan",pivoting:e});if(s.singular)return{steps:s.steps,inverse:null,singular:!0};const h=_t(s.matrix,o,2*o);return{steps:s.steps.concat([{kind:"done",matrix:X(s.matrix),coeffCols:o,varOrder:s.varOrder.slice(),caption:{en:"The right block is A⁻¹.",hu:"A jobb oldali blokk az A⁻¹."}}]),inverse:h,singular:!1}}function zt(n,e="partial"){const o=n.length,i=ln(n.map(l=>l.slice()),o,{method:"gauss",pivoting:e}),s=gn(i);return{steps:i.steps,determinant:s}}function D(n,e,o,i,s){const h=e.length,l=[];for(let d=0;d<h;d++){const a=new Array(h+1).fill(R);a[d]=e[d],d<h-1&&(a[d+1]=o[d]),d>0&&(a[d-1]=d<=s?R:n[d-1]),a[h]=i[d],l.push(a)}return l}function qt(n){const e=n.d.length,o=n.a.slice(),i=n.d.slice(),s=n.c.slice(),h=n.b.slice(),l=[];l.push({kind:"init",matrix:D(o,i,s,h,0),coeffCols:e,varOrder:Array.from({length:e},(a,u)=>u),caption:{en:"Initial tridiagonal system (A | b).",hu:"Kiindulási tridiagonális rendszer (A | b)."}});for(let a=1;a<e;a++){if(W(i[a-1]))return l.push({kind:"pivot-select",matrix:D(o,i,s,h,a-1),coeffCols:e,varOrder:Array.from({length:e},(k,$)=>$),pivot:[a-1,a-1],caption:{en:`Zero pivot d${a} — the Thomas algorithm cannot continue without pivoting.`,hu:`Nulla főelem d${a} — a Thomas-algoritmus főelemkiválasztás nélkül nem folytatható.`}}),{steps:l,solution:null,singular:!0};const u=o[a-1].div(i[a-1]);i[a]=i[a].sub(u.mul(s[a-1])),h[a]=h[a].sub(u.mul(h[a-1])),l.push({kind:"eliminate",matrix:D(o,i,s,h,a),coeffCols:e,varOrder:Array.from({length:e},(k,$)=>$),pivot:[a-1,a-1],multiplier:u,changed:[[a,a-1],[a,a],[a,e]],caption:{en:`Eliminate a${a}: m = a${a}/d${a} = ${z(u)}; update d${a+1} and b${a+1}.`,hu:`a${a} kiküszöbölése: m = a${a}/d${a} = ${z(u)}; d${a+1} és b${a+1} frissítése.`}})}if(W(i[e-1]))return{steps:l,solution:null,singular:!0};const d=new Array(e);d[e-1]=h[e-1].div(i[e-1]),l.push({kind:"back-sub",matrix:D(o,i,s,h,e-1),coeffCols:e,varOrder:Array.from({length:e},(a,u)=>u),pivot:[e-1,e-1],caption:{en:`Back-substitution: x${e} = b${e}/d${e} = ${z(d[e-1])}.`,hu:`Visszahelyettesítés: x${e} = b${e}/d${e} = ${z(d[e-1])}.`}});for(let a=e-2;a>=0;a--)d[a]=h[a].sub(s[a].mul(d[a+1])).div(i[a]),l.push({kind:"back-sub",matrix:D(o,i,s,h,e-1),coeffCols:e,varOrder:Array.from({length:e},(u,k)=>k),pivot:[a,a],caption:{en:`Back-substitution: x${a+1} = (b${a+1} − c${a+1}·x${a+2})/d${a+1} = ${z(d[a])}.`,hu:`Visszahelyettesítés: x${a+1} = (b${a+1} − c${a+1}·x${a+2})/d${a+1} = ${z(d[a])}.`}});return{steps:l,solution:d,singular:!1}}const on=[{id:"ex3-22",ref:"Example 3.22",name:{en:"Gaussian elimination",hu:"Gauss-elimináció"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[-11,-8,27,28],note:{en:"Solves cleanly without pivoting; solution (-3, 2, 4, -2).",hu:"Főelemkiválasztás nélkül is megoldható; megoldás (-3, 2, 4, -2)."}},{id:"ex3-24",ref:"Example 3.24 / 3.27",name:{en:"Zero pivot (needs pivoting)",hu:"Nulla főelem (kell csere)"},A:[[2,-1,0,-3],[2,-1,1,5],[-3,1,1,-2],[2,4,0,-1]],b:[8,2,-5,21],note:{en:"Without pivoting it stalls on a zero pivot. Switch pivoting to Partial to finish — solution (4, 3, 2, -1).",hu:"Főelemkiválasztás nélkül nulla főelemen elakad. Válts Részleges főelemkiválasztásra — megoldás (4, 3, 2, -1)."}},{id:"ex3-25",ref:"Example 3.25",name:{en:"Ill-conditioned 2×2",hu:"Rosszul kondicionált 2×2"},A:[["1/5000","-61/2"],["253/50","-21/20"]],b:["-6099/100","2509/10"],note:{en:"Exact solution (50, 2). With finite-digit arithmetic, dividing by the tiny pivot 0.0002 blows up the error — partial pivoting fixes it.",hu:"Pontos megoldás (50, 2). Véges jegyű aritmetikával a kicsi 0.0002 főelemmel való osztás felnagyítja a hibát — a részleges főelemkiválasztás megoldja."}},{id:"ex3-33a",ref:"§3.3 Exercise 1(a)",name:{en:"Exercise 3×3",hu:"Feladat 3×3"},A:[[2,2,-2],[-1,3,0],[4,2,-3]],b:[-4,-11,-1]}],Tt=[{id:"ex3-38",ref:"Example 3.38",name:{en:"Invert a 3×3",hu:"3×3 invertálása"},A:[[1,0,2],[-1,1,0],[-2,0,-1]],b:[0,0,0],note:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]].",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]]."}},{id:"ex3-39",ref:"Example 3.39",name:{en:"Determinant = 114",hu:"Determináns = 114"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[0,0,0,0],note:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"ex3-7-1a",ref:"§3.7 Exercise 1(a)",name:{en:"Invert 3×3 (exercise)",hu:"3×3 invertálása (feladat)"},A:[[-1,1,2],[-2,1,0],[0,1,-1]],b:[0,0,0]}],B=[{id:"ex3-5-1",ref:"§3.5 Exercise 1",name:{en:"6×6 tridiagonal",hu:"6×6 tridiagonális"},a:["1/2","1/2","1/2","1/2","1/2"],d:[1,4,2,4,2,1],c:["-1/2","-1/2","-1/2","-1/2","-1/2"],b:["3/2",-4,2,-4,2,"-1/2"]},{id:"tri-small",ref:"Warm-up",name:{en:"4×4 tridiagonal",hu:"4×4 tridiagonális"},a:[-1,-1,-1],d:[4,4,4,4],c:[-1,-1,-1],b:[5,5,10,23]}];function It(n,e,o){return!!n&&n[0]===e&&n[1]===o}function $n({matrix:n,coeffCols:e,pivot:o,changed:i,varOrder:s,bracket:h=!0}){const l=new Set((i??[]).map(([a,u])=>`${a},${u}`)),d=s!==void 0&&s.some((a,u)=>a!==u);return t.jsxs("div",{className:"matrix-view",children:[t.jsx("table",{className:`matrix${h?" bracketed":""}`,children:t.jsx("tbody",{children:n.map((a,u)=>t.jsx("tr",{children:a.map((k,$)=>{const b=[];return It(o,u,$)?b.push("cell-pivot"):l.has(`${u},${$}`)&&b.push("cell-changed"),e!==void 0&&$===e&&b.push("rhs-sep"),t.jsx("td",{className:b.join(" ")||void 0,children:t.jsx(J,{tex:an(k)})},$)})},u))})}),d&&t.jsx("div",{className:"var-order",children:s.map((a,u)=>t.jsxs("span",{children:["col ",u+1," → ",t.jsx(J,{tex:`x_{${a+1}}`})]},u))})]})}function Mt({values:n,onChange:e,label:o,invalid:i}){var h;const s=((h=n[0])==null?void 0:h.length)??0;return t.jsxs("label",{className:"field",children:[o,t.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:`repeat(${s}, auto)`},children:n.map((l,d)=>l.map((a,u)=>t.jsx("input",{type:"text",inputMode:"decimal",value:a,"aria-label":`a${d+1}${u+1}`,style:i!=null&&i(d,u)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:k=>e(d,u,k.target.value)},`${d}-${u}`)))})]})}function H({values:n,onChange:e,label:o,invalid:i,horizontal:s=!1}){return t.jsxs("label",{className:"field",children:[o,t.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:s?`repeat(${n.length}, auto)`:"auto"},children:n.map((h,l)=>t.jsx("input",{type:"text",inputMode:"decimal",value:h,"aria-label":`b${l+1}`,style:i!=null&&i(l)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:d=>e(l,d.target.value)},l))})]})}const un=n=>n.map(e=>e.map(String)),G=n=>n.map(String),xn=n=>Array.from({length:n},()=>"0"),Gt=n=>Array.from({length:n},()=>xn(n)),V=n=>n.map(e=>Y(e.trim()===""?0:e.trim())),St=n=>n.map(V);function U(n){return n==="solve"?on:Tt}function Wt({initial:n}){const{t:e,pick:o,lang:i}=S(),[s,h]=A.useState((n==null?void 0:n.mode)??"solve"),[l,d]=A.useState((n==null?void 0:n.method)??"gauss"),[a,u]=A.useState((n==null?void 0:n.pivot)??"none"),k=A.useMemo(()=>{const m=(n==null?void 0:n.mode)??"solve";if(m==="tridiagonal")return{presetId:(B.find(_=>_.id===(n==null?void 0:n.presetId))??B[0]).id};const x=U(m);return{presetId:(x.find(p=>p.id===(n==null?void 0:n.presetId))??x[0]).id}},[n==null?void 0:n.mode,n==null?void 0:n.presetId]),[$,b]=A.useState(k.presetId),I=on[0],[q,r]=A.useState(()=>{const m=(n==null?void 0:n.mode)??"solve",x=U(m),f=x.find(p=>p.id===(n==null?void 0:n.presetId))??x[0]??I;return un(f.A)}),[j,T]=A.useState(()=>{const m=on.find(x=>x.id===(n==null?void 0:n.presetId))??I;return G(m.b)}),[c,v]=A.useState(()=>{const m=B.find(x=>x.id===(n==null?void 0:n.presetId))??B[0];return{a:G(m.a),d:G(m.d),c:G(m.c),b:G(m.b)}}),[w,y]=A.useState(0),[E,Z]=A.useState(!1);function nn(m,x){if(b(x),m==="tridiagonal"){const _=B.find(L=>L.id===x)??B[0];v({a:G(_.a),d:G(_.d),c:G(_.c),b:G(_.b)});return}const f=U(m),p=f.find(_=>_.id===x)??f[0];p&&(r(un(p.A)),T(G(p.b)))}function kn(m){h(m),m==="inverse"?(d("gauss-jordan"),(a==="complete"||a==="scaled")&&u("none")):m==="determinant"&&d("gauss"),m==="tridiagonal"?nn(m,B[0].id):nn(m,U(m)[0].id)}function vn(m){r(x=>{const f=Gt(m);for(let p=0;p<Math.min(m,x.length);p++)for(let _=0;_<Math.min(m,x[p].length);_++)f[p][_]=x[p][_];return f}),T(x=>{const f=xn(m);for(let p=0;p<Math.min(m,x.length);p++)f[p]=x[p];return f}),b("custom")}const C=A.useMemo(()=>{try{if(s==="tridiagonal"){const f=V(c.a),p=V(c.d),_=V(c.c),L=V(c.b);if(f.length!==p.length-1||_.length!==p.length-1||L.length!==p.length)return{steps:[],error:"dimension mismatch"};const F=qt({a:f,d:p,c:_,b:L});return{steps:F.steps,error:null,solution:F.solution,singular:F.singular}}const m=St(q);if(s==="solve"){const f=wt(m,V(j),{method:l,pivoting:a});return{steps:f.steps,error:null,solution:f.solution,determinant:f.determinant,singular:f.singular}}if(s==="inverse"){const f=jt(m,a==="partial"?"partial":"none");return{steps:f.steps,error:null,inverse:f.inverse,singular:f.singular}}const x=zt(m,a);return{steps:x.steps,error:null,determinant:x.determinant}}catch{return{steps:[],error:"invalid"}}},[s,l,a,q,j,c]);A.useEffect(()=>{y(0),Z(!1)},[C]);const en=A.useRef(null);A.useEffect(()=>{if(E)return en.current=window.setInterval(()=>{y(m=>m>=C.steps.length-1?(Z(!1),m):m+1)},900),()=>{en.current&&window.clearInterval(en.current)}},[E,C.steps.length]);const P=C.steps,O=Math.min(w,Math.max(0,P.length-1)),N=P[O],_n=q.length,mn=s==="solve",hn=s==="solve"||s==="determinant"||s==="inverse",An=s==="inverse"?["none","partial"]:["none","partial","complete","scaled"],dn=s==="tridiagonal"?B:U(s),K=dn.find(m=>m.id===$)??null;return t.jsxs("div",{className:"grid-2",children:[t.jsxs("div",{className:"card stack",children:[t.jsxs("div",{className:"row",children:[t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.mode"),t.jsxs("select",{value:s,onChange:m=>kn(m.target.value),children:[t.jsx("option",{value:"solve",children:e("lab.mode.solve")}),t.jsx("option",{value:"inverse",children:e("lab.mode.inverse")}),t.jsx("option",{value:"determinant",children:e("lab.mode.determinant")}),t.jsx("option",{value:"tridiagonal",children:e("lab.mode.tridiagonal")})]})]}),t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.preset"),t.jsxs("select",{value:$,onChange:m=>nn(s,m.target.value),children:[dn.map(m=>t.jsxs("option",{value:m.id,children:[m.ref," — ",o(m.name)]},m.id)),t.jsx("option",{value:"custom",children:e("lab.preset.custom")})]})]})]}),(mn||hn)&&t.jsxs("div",{className:"row",children:[mn&&t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.method"),t.jsxs("select",{value:l,onChange:m=>d(m.target.value),children:[t.jsx("option",{value:"gauss",children:e("method.gauss")}),t.jsx("option",{value:"gauss-jordan",children:e("method.gaussJordan")})]})]}),hn&&t.jsxs("label",{className:"field",style:{flex:1},children:[e("lab.pivoting"),t.jsx("select",{value:a,onChange:m=>u(m.target.value),children:An.map(m=>t.jsx("option",{value:m,children:e(`pivot.${m}`)},m))})]})]}),K&&"note"in K&&K.note&&t.jsx("div",{className:"caption",children:o(K.note)}),s==="inverse"&&t.jsx("div",{className:"muted",style:{fontSize:"0.82rem"},children:e("lab.note.inverse")}),s==="tridiagonal"?t.jsxs("div",{className:"stack",children:[t.jsx(H,{label:i==="hu"?"Főátló d":"Diagonal d",values:c.d,horizontal:!0,onChange:(m,x)=>{b("custom"),v(f=>({...f,d:f.d.map((p,_)=>_===m?x:p)}))}}),t.jsx(H,{label:i==="hu"?"Felső átló c":"Super-diagonal c",values:c.c,horizontal:!0,onChange:(m,x)=>{b("custom"),v(f=>({...f,c:f.c.map((p,_)=>_===m?x:p)}))}}),t.jsx(H,{label:i==="hu"?"Alsó átló a":"Sub-diagonal a",values:c.a,horizontal:!0,onChange:(m,x)=>{b("custom"),v(f=>({...f,a:f.a.map((p,_)=>_===m?x:p)}))}}),t.jsx(H,{label:e("lab.vectorB"),values:c.b,horizontal:!0,onChange:(m,x)=>{b("custom"),v(f=>({...f,b:f.b.map((p,_)=>_===m?x:p)}))}})]}):t.jsxs("div",{className:"stack",children:[t.jsxs("label",{className:"field",children:[e("lab.size"),t.jsx("select",{value:_n,onChange:m=>vn(Number(m.target.value)),children:[2,3,4,5].map(m=>t.jsxs("option",{value:m,children:[m," × ",m]},m))})]}),t.jsx(Mt,{label:e("lab.matrixA"),values:q,onChange:(m,x,f)=>{b("custom"),r(p=>p.map((_,L)=>L===m?_.map((F,yn)=>yn===x?f:F):_))}}),s==="solve"&&t.jsx(H,{label:e("lab.vectorB"),values:j,onChange:(m,x)=>{b("custom"),T(f=>f.map((p,_)=>_===m?x:p))}})]})]}),t.jsx("div",{className:"card stack",children:C.error||!N?t.jsx("p",{className:"feedback err",children:i==="hu"?"Érvénytelen bemenet.":"Invalid input."}):t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"caption",children:o(N.caption)}),t.jsx($n,{matrix:N.matrix,coeffCols:s==="inverse"?void 0:N.coeffCols,pivot:N.pivot,changed:N.changed,varOrder:N.varOrder}),t.jsxs("div",{className:"kbd-controls",children:[t.jsxs("button",{className:"icon-btn",onClick:()=>y(0),disabled:O===0,children:["⏮ ",e("lab.first")]}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(m=>Math.max(0,m-1)),disabled:O===0,children:["◀ ",e("lab.prev")]}),t.jsx("button",{className:"btn",onClick:()=>Z(m=>!m),children:E?`⏸ ${e("lab.pause")}`:`▶ ${e("lab.play")}`}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(m=>Math.min(P.length-1,m+1)),disabled:O>=P.length-1,children:[e("lab.next")," ▶"]}),t.jsxs("button",{className:"icon-btn",onClick:()=>y(P.length-1),disabled:O>=P.length-1,children:[e("lab.last")," ⏭"]})]}),t.jsxs("div",{className:"row",children:[t.jsx("span",{className:"muted",style:{fontSize:"0.85rem",minWidth:110},children:e("lab.stepOf",{a:O+1,b:P.length})}),t.jsx("div",{className:"progress",children:t.jsx("div",{style:{width:`${(O+1)/P.length*100}%`}})})]}),t.jsx(Et,{run:C,mode:s})]})})]})}function Et({run:n,mode:e}){const{t:o}=S();return n.singular&&e!=="determinant"?t.jsx("div",{className:"caption",children:o("lab.singular")}):t.jsxs("div",{className:"card",style:{background:"var(--bg-sunken)"},children:[t.jsx("div",{className:"section-eyebrow",children:o("lab.result")}),(e==="solve"||e==="tridiagonal")&&n.solution&&t.jsxs("div",{className:"result-line",style:{marginTop:8},children:[t.jsxs("strong",{children:[o("lab.solution"),":"]}),n.solution.map((i,s)=>t.jsx("span",{className:"pill",children:t.jsx(J,{tex:`x_{${s+1}} = ${an(i)}`})},s))]}),(e==="solve"||e==="determinant")&&n.determinant&&t.jsxs("div",{className:"result-line",style:{marginTop:8},children:[t.jsxs("strong",{children:[o("lab.determinant"),":"]}),t.jsx("span",{className:"pill",children:t.jsx(J,{tex:`\\det(A) = ${an(n.determinant)}`})})]}),e==="inverse"&&n.inverse&&t.jsxs("div",{style:{marginTop:8},children:[t.jsxs("strong",{children:[o("lab.inverse"),":"]}),t.jsx($n,{matrix:n.inverse})]})]})}const Q=[{id:"q-tri-solve",kind:"vector",prompt:{en:"Solve this triangular system by backward substitution (enter x₁, x₂, x₃, x₄):",hu:"Oldd meg ezt a háromszög rendszert visszahelyettesítéssel (add meg x₁, x₂, x₃, x₄):"},tex:"\\begin{array}{rcrcrcrcr} 2x_1 &-& x_2 &+& 3x_3 &+& x_4 &=& 3\\\\ && 3x_2 &-& x_3 &+& 2x_4 &=& 13\\\\ &&&& 2x_3 &-& x_4 &=& -2\\\\ &&&&&& 3x_4 &=& 12 \\end{array}",answer:["-1","2","1","4"],solution:{en:"x₄ = 4, then x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1.",hu:"x₄ = 4, majd x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1."}},{id:"q-det-114",kind:"numeric",prompt:{en:"What is det(A) for the coefficient matrix of Example 3.39 (pivots 1, 3, 1, 38, no swaps)?",hu:"Mennyi det(A) a 3.39. példa együtthatómátrixára (főelemek 1, 3, 1, 38, csere nélkül)?"},answer:"114",solution:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"q-dd-invertible",kind:"truefalse",prompt:{en:"Every (row) diagonally dominant matrix is invertible.",hu:"Minden (soronként) diagonálisan domináns mátrix invertálható."},answer:!0,solution:{en:"True. Diagonal dominance implies Ax = 0 has only the trivial solution, so A is nonsingular.",hu:"Igaz. A diagonális dominanciából következik, hogy Ax = 0-nak csak triviális megoldása van, így A reguláris."}},{id:"q-partial-pivot",kind:"choice",prompt:{en:"In column 1 the entries (top to bottom) are 2, 2, −3, 2. Which row does partial pivoting move to the top?",hu:"Az 1. oszlop elemei (fentről le) 2, 2, −3, 2. Melyik sort viszi felülre a részleges főelemkiválasztás?"},choices:[{en:"Row 1 (value 2)",hu:"1. sor (érték 2)"},{en:"Row 3 (value −3)",hu:"3. sor (érték −3)"},{en:"The first nonzero row",hu:"Az első nemnulla sor"},{en:"No swap is needed",hu:"Nem kell csere"}],answer:1,solution:{en:"Partial pivoting picks the largest magnitude, |−3| = 3, so row 3 moves up.",hu:"A részleges főelemkiválasztás a legnagyobb abszolút értéket választja, |−3| = 3, így a 3. sor kerül felülre."}},{id:"q-gauss-cost",kind:"choice",prompt:{en:"What is the leading-order operation count of Gaussian elimination?",hu:"Mi a Gauss-elimináció vezető rendű műveletigénye?"},choices:[{en:"n²/2",hu:"n²/2"},{en:"n³/3",hu:"n³/3"},{en:"n³/2",hu:"n³/2"},{en:"5n − 4",hu:"5n − 4"}],answer:1,solution:{en:"Forward elimination plus back-substitution is n³/3 + O(n²) multiplications/divisions.",hu:"Az előre elimináció és a visszahelyettesítés együtt n³/3 + O(n²) szorzás/osztás."}},{id:"q-gj-cost",kind:"truefalse",prompt:{en:"Gauss–Jordan elimination is asymptotically cheaper than Gaussian elimination.",hu:"A Gauss–Jordan-elimináció aszimptotikusan olcsóbb, mint a Gauss-elimináció."},answer:!1,solution:{en:"False. Gauss–Jordan costs ≈ n³/2, more than Gaussian elimination’s n³/3.",hu:"Hamis. A Gauss–Jordan költsége ≈ n³/2, több, mint a Gauss-elimináció n³/3-a."}},{id:"q-thomas-cost",kind:"choice",prompt:{en:"How many multiplications/divisions does the Thomas (tridiagonal) algorithm need?",hu:"Hány szorzás/osztás kell a Thomas- (tridiagonális) algoritmushoz?"},choices:[{en:"n³/3",hu:"n³/3"},{en:"n²/2",hu:"n²/2"},{en:"5n − 4",hu:"5n − 4"},{en:"2ⁿ",hu:"2ⁿ"}],answer:2,solution:{en:"A tridiagonal solve is linear: 5n − 4 multiplications/divisions.",hu:"A tridiagonális megoldás lineáris: 5n − 4 szorzás/osztás."}},{id:"q-inv-entry",kind:"numeric",prompt:{en:"For A of Example 3.38, what is the (1,1) entry of A⁻¹? (Enter a fraction like -1/3.)",hu:"A 3.38. példa A mátrixára mi az A⁻¹ (1,1) eleme? (Adj meg törtet, pl. -1/3.)"},answer:"-1/3",solution:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], so the (1,1) entry is −1/3.",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], tehát az (1,1) elem −1/3."}},{id:"q-posdef",kind:"choice",prompt:{en:"A symmetric matrix is positive definite if and only if…",hu:"Egy szimmetrikus mátrix akkor és csak akkor pozitív definit, ha…"},choices:[{en:"all its entries are positive",hu:"minden eleme pozitív"},{en:"it is diagonally dominant",hu:"diagonálisan domináns"},{en:"all leading principal minors are positive",hu:"minden bal felső főminora pozitív"},{en:"its determinant is positive",hu:"a determinánsa pozitív"}],answer:2,solution:{en:"Sylvester’s criterion: positive definite ⇔ every leading principal minor is positive.",hu:"Sylvester-kritérium: pozitív definit ⇔ minden bal felső főminor pozitív."}}];function cn(n,e){try{return Y(n.trim()).equals(Y(e.trim()))}catch{return!1}}function fn(n,e){switch(n.kind){case"choice":return e.choice===n.answer;case"truefalse":return e.bool===n.answer;case"numeric":return cn(e.text??"",n.answer);case"vector":{const o=(e.text??"").split(/[,\s]+/).filter(Boolean);return o.length!==n.answer.length?!1:o.every((i,s)=>cn(i,n.answer[s]))}}}function Pt({item:n,onAnswered:e}){const{t:o,pick:i}=S(),[s,h]=A.useState({}),[l,d]=A.useState(!1),[a,u]=A.useState(!1),k=l&&fn(n,s);function $(){d(!0),e(fn(n,s))}return t.jsxs("div",{className:"card stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:o("quiz.title")}),t.jsx("p",{style:{marginBottom:6},children:i(n.prompt)}),n.tex&&t.jsx(J,{tex:n.tex,block:!0})]}),n.kind==="choice"&&t.jsx("div",{className:"choices",children:n.choices.map((b,I)=>{let q="choice";return s.choice===I&&(q+=" selected"),l&&I===n.answer&&(q+=" correct"),l&&s.choice===I&&I!==n.answer&&(q+=" wrong"),t.jsx("button",{className:q,onClick:()=>!l&&h({choice:I}),disabled:l,children:i(b)},I)})}),n.kind==="truefalse"&&t.jsxs("div",{className:"seg",children:[t.jsx("button",{className:s.bool===!0?"active":"",onClick:()=>!l&&h({bool:!0}),children:o("quiz.true")}),t.jsx("button",{className:s.bool===!1?"active":"",onClick:()=>!l&&h({bool:!1}),children:o("quiz.false")})]}),(n.kind==="numeric"||n.kind==="vector")&&t.jsx("input",{type:"text",placeholder:n.kind==="vector"?"x₁, x₂, …":o("quiz.placeholder"),value:s.text??"",onChange:b=>h({text:b.target.value}),disabled:l,style:{maxWidth:280}}),t.jsxs("div",{className:"row",children:[l?t.jsx("span",{className:k?"feedback ok":"feedback err",children:o(k?"quiz.correct":"quiz.incorrect")}):t.jsx("button",{className:"btn",onClick:$,children:o("quiz.check")}),t.jsx("button",{className:"btn secondary",onClick:()=>u(b=>!b),children:o(a?"quiz.hideSolution":"quiz.showSolution")})]}),a&&t.jsx("div",{className:"caption",children:i(n.solution)})]})}function Bt(){const{t:n}=S(),[e,o]=A.useState(0),[i,s]=A.useState({}),h=Q[e],l=Object.values(i).filter(Boolean).length;return t.jsxs("div",{className:"stack",children:[t.jsxs("div",{className:"row",style:{justifyContent:"space-between"},children:[t.jsx("span",{className:"muted",children:n("quiz.questionOf",{a:e+1,b:Q.length})}),t.jsxs("span",{className:"pill",children:[n("quiz.score"),": ",l," / ",Object.keys(i).length]})]}),t.jsx(Pt,{item:h,onAnswered:d=>s(a=>({...a,[e]:d}))},h.id),t.jsxs("div",{className:"row",children:[t.jsxs("button",{className:"icon-btn",onClick:()=>o(d=>Math.max(0,d-1)),disabled:e===0,children:["◀ ",n("quiz.prev")]}),t.jsxs("button",{className:"icon-btn",onClick:()=>o(d=>Math.min(Q.length-1,d+1)),disabled:e===Q.length-1,children:[n("quiz.next")," ▶"]})]})]})}const Ot=[...bn.map(n=>({id:n.id,no:n.number,title:n.title,blurb:n.summary})),{id:"lab",no:"3·lab",title:{en:"Elimination Lab",hu:"Eliminációs labor"},blurb:{en:"Step through Gaussian / Gauss–Jordan elimination interactively.",hu:"Lépkedj végig a Gauss- / Gauss–Jordan-eliminációi lépéseken."}},{id:"quiz",no:"3·quiz",title:{en:"Quiz",hu:"Kvíz"},blurb:{en:"Check your understanding.",hu:"Ellenőrizd a tudásod."}}];function Ht(){const{t:n,lang:e}=S(),o=jn();return A.useEffect(()=>{let i=decodeURIComponent(o.hash.replace(/^#/,""));if(!i){const s=o.pathname.match(/\/lessons\/([^/]+)/);s?i=s[1]:/\/lab$/.test(o.pathname)?i="lab":/\/quiz$/.test(o.pathname)&&(i="quiz")}i&&requestAnimationFrame(()=>{var s;return(s=document.getElementById(i))==null?void 0:s.scrollIntoView()})},[o.pathname,o.hash]),t.jsxs("div",{className:"app-shell ch-linear-systems",children:[t.jsx(qn,{sections:Ot}),t.jsx("main",{children:t.jsxs("div",{className:"container",children:[t.jsxs("section",{className:"hero",id:"top",children:[t.jsx("span",{className:"section-eyebrow",children:n("app.subtitle")}),t.jsx("h1",{children:n("app.title")}),t.jsx("p",{children:n("home.tagline")}),t.jsx("p",{style:{marginTop:4},children:n("home.lead")})]}),bn.map(i=>t.jsxs("section",{id:i.id,className:"ls-section",children:[t.jsx(On,{section:i}),$t(i.id).map(s=>t.jsx(Tn,{snippets:s.snippets,caption:s.caption},s.id)),t.jsx(Cn,{sectionNumber:i.number})]},i.id)),t.jsxs("section",{id:"lab",className:"ls-section stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:n("nav.lab")}),t.jsx("h1",{style:{margin:"4px 0 2px"},children:n("nav.lab")})]}),t.jsx(Wt,{initial:{}})]}),t.jsxs("section",{id:"quiz",className:"ls-section stack",children:[t.jsxs("div",{children:[t.jsx("span",{className:"section-eyebrow",children:n("nav.quiz")}),t.jsx("h1",{style:{margin:"4px 0 2px"},children:n("quiz.title")})]}),t.jsx(Bt,{})]})]})}),t.jsx("footer",{className:"footer",children:t.jsx("div",{className:"container",children:e==="hu"?"Numerikus analízis · 3. fejezet — interaktív tananyag.":"Numerical Analysis · Chapter 3 — interactive companion."})})]})}export{Ht as default};
