...
`,Oe=`program back_substitution_demo
  implicit none
...
`,Ce=`function back_substitution(U, b)
    n = length(b)
    x = zeros(n)
    for i in n:-1:1
...
`,Ve=`function backSubstitution(U, b) {
  const n = b.length;
...
`,He=`function x = back_substitution(U, b)
...
`,Fe=`back_substitution <- function(U, b) {
  # Solve an upper-triangular system U x = b.
  n <- length(b)
  x <- numeric(n)
  for (i in n:1) {
...
b <- c(1, 8, 9)
print(back_substitution(U, b))    # [1, 2, 3]
`,Ue=`// Solve an upper-triangular system U x = b.
...
`,Re=`backSubstitution[U_, b_] := Module[{n = Length[b], x},
   x = ConstantArray[0., n];
...
`,Le=`program gauss_complete_demo
  implicit none
  integer, parameter :: n = 3
...
`,Qe=`function gauss_complete_pivot(A, b)
...
`,Ye=`// Gaussian elimination with complete (row + column) pivoting.
...
`,Ze=`function x = gauss_complete_pivot(A, b)
...
`,tt=`gauss_complete_pivot <- function(A, b) {
  # Gaussian elimination with complete (row+column) pivoting.
  # Returns x (column swaps are undone before returning).
...
`,nt=`// Gaussian elimination with complete (row + column) pivoting.
...
`,it=`gaussComplete[Ain_, bin_] := Module[
...
`,st=`program gauss_elimination_demo
  implicit none
  integer, parameter :: n = 3
...
`,rt=`function gauss_elimination(A, b)
    n = length(b)
    M = [A b]                       # augmented [A | b]
...
`,lt=`function gaussElimination(A, b) {
  const n = b.length;
...
`,mt=`function x = gauss_elimination(A, b)
...
`,dt=`gauss_elimination <- function(A, b) {
  # Naive Gaussian elimination (no pivoting) + back-substitution.
  n <- length(b)
...
b <- c(8, -11, -3)
print(gauss_elimination(A, b))    # [2, 3, -1]
`,ut=`// Naive Gaussian elimination (no pivoting) + back-substitution.
...
`,ct=`gaussElimination[A_, b_] := Module[{n = Length[b], M, x, f},
...
`,gt=`// Solve A x = b by Gauss-Jordan elimination.
...
`,kt=`function x = gauss_jordan(A, b)
...
`,_t=`// Solve A x = b by Gauss-Jordan elimination (reduced row echelon form).
...
`,At=`gaussJordan[Ain_, bin_] := Module[{M = N@MapThread[Append, {Ain, bin}], n = Length[bin], p, f},
   Do[
...
`,jt=`function gauss_partial_pivot(A, b)
...
`,qt=`// Gaussian elimination with partial (row) pivoting.
...
`,It=`function x = gauss_partial_pivot(A, b)
...
`,Mt=`gauss_partial_pivot <- function(A, b) {
  # Solve A x = b by Gaussian elimination with partial (row) pivoting.
  A <- matrix(as.numeric(A), nrow = nrow(A))
  b <- as.numeric(b)
  n <- length(b)
...
`,Gt=`// Gaussian elimination with partial (row) pivoting.
...
`,Wt=`gaussPartial[Ain_, bin_] := Module[{A = N[Ain], b = N[bin], n = Length[bin], p, f, x},
   Do[
...
`,Pt=`program matrix_inverse_demo
  implicit none
  integer, parameter :: n = 3
...
`,Ot=`function Ainv = matrix_inverse(A)
...
`,Vt=`// Matrix inverse via Gauss-Jordan on the augmented matrix [A | I].
...
`,Ht=`inverse[Ain_] := Module[{A = N[Ain], n = Length[Ain], M, p, d},
...
`,Xt=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
d <- c(2, 4, 6, 13)
print(thomas(a, b, c, d))
`,Yt=`// Thomas algorithm for a tridiagonal system (a sub-, b diag, c super-, d rhs).
...
`,Zt=`thomas[a_, b_, cIn_, dIn_] := Module[{n = Length[dIn], c = cIn, d = dIn, x, m},
   c[[1]] /= b[[1]]; d[[1]] /= b[[1]];
   Do[m = b[[i]] - a[[i]] c[[i - 1]];
      If[i < n, c[[i]] /= m];
...
`,en=Object.assign({"./back-substitution.cpp":Ne,"./back-substitution.f90":Oe,"./back-substitution.go":Je,"./back-substitution.jl":Ce,"./back-substitution.js":Ve,"./back-substitution.m":He,"./back-substitution.py":De,"./back-substitution.r":Fe,"./back-substitution.rs":Ue,"./back-substitution.wl":Re,"./gauss-complete.cpp":Xe,"./gauss-complete.f90":Le,"./gauss-complete.go":Ke,"./gauss-complete.jl":Qe,"./gauss-complete.js":Ye,"./gauss-complete.m":Ze,"./gauss-complete.py":et,"./gauss-complete.r":tt,"./gauss-complete.rs":nt,"./gauss-complete.wl":it,"./gauss-elimination.cpp":at,"./gauss-elimination.f90":st,"./gauss-elimination.go":ot,"./gauss-elimination.jl":rt,"./gauss-elimination.js":lt,"./gauss-elimination.m":mt,"./gauss-elimination.py":ht,"./gauss-elimination.r":dt,"./gauss-elimination.rs":ut,"./gauss-elimination.wl":ct,"./gauss-jordan.cpp":ft,"./gauss-jordan.f90":bt,"./gauss-jordan.go":pt,"./gauss-jordan.jl":$t,"./gauss-jordan.js":gt,"./gauss-jordan.m":kt,"./gauss-jordan.py":vt,"./gauss-jordan.r":xt,"./gauss-jordan.rs":_t,"./gauss-jordan.wl":At,"./gauss-partial.cpp":yt,"./gauss-partial.f90":wt,"./gauss-partial.go":zt,"./gauss-partial.jl":jt,"./gauss-partial.js":qt,"./gauss-partial.m":It,"./gauss-partial.py":Tt,"./gauss-partial.r":Mt,"./gauss-partial.rs":Gt,"./gauss-partial.wl":Wt,"./matrix-inverse.cpp":St,"./matrix-inverse.f90":Pt,"./matrix-inverse.go":Et,"./matrix-inverse.jl":Bt,"./matrix-inverse.js":Nt,"./matrix-inverse.m":Ot,"./matrix-inverse.py":Jt,"./matrix-inverse.r":Ct,"./matrix-inverse.rs":Vt,"./matrix-inverse.wl":Ht,"./tridiagonal.cpp":Dt,"./tridiagonal.f90":Ft,"./tridiagonal.go":Ut,"./tridiagonal.jl":Rt,"./tridiagonal.js":Xt,"./tridiagonal.m":Lt,"./tridiagonal.py":Kt,"./tridiagonal.r":Qt,"./tridiagonal.rs":Yt,"./tridiagonal.wl":Zt}),M=(e,t)=>en[`./${e}.${t}`],tn={"back-substitution":{en:"Back-substitution (upper-triangular system)",hu:"Visszahelyettesítés (felső háromszögű rendszer)"},"gauss-elimination":{en:"Gaussian elimination (no pivoting)",hu:"Gauss-elimináció (főelemkiválasztás nélkül)"},"gauss-partial":{en:"Gaussian elimination — partial pivoting",hu:"Gauss-elimináció — részleges főelemkiválasztás"},"gauss-complete":{en:"Gaussian elimination — complete pivoting",hu:"Gauss-elimináció — teljes főelemkiválasztás"},"gauss-jordan":{en:"Gauss–Jordan elimination",hu:"Gauss–Jordan-elimináció"},"matrix-inverse":{en:"Matrix inverse (Gauss–Jordan on [A | I])",hu:"Mátrixinverz (Gauss–Jordan az [A | I] mátrixon)"},tridiagonal:{en:"Tridiagonal solver (Thomas algorithm)",hu:"Tridiagonális megoldó (Thomas-algoritmus)"}},nn=e=>({id:e,caption:tn[e],snippets:{matlab:M(e,"m"),python:M(e,"py"),cpp:M(e,"cpp"),julia:M(e,"jl"),rust:M(e,"rs"),fortran:M(e,"f90"),wolfram:M(e,"wl"),javascript:M(e,"js"),go:M(e,"go"),r:M(e,"r")}}),an={s33:["back-substitution","gauss-elimination","gauss-partial","gauss-complete","gauss-jordan","matrix-inverse","tridiagonal"]};function sn(e){return(an[e]??[]).map(nn)}function Y(e,t){return new se(e)}const X=new se(0),be=new se(1);function W(e){return e.n===0}function ie(e){if(e.n===0)return"0";const t=e.s<0?"-":"";return e.d===1?`${t}${e.n}`:`${t}\\frac{${e.n}}{${e.d}}`}function j(e){if(e.n===0)return"0";const t=e.s<0?"-":"";return e.d===1?`${t}${e.n}`:`${t}${e.n}/${e.d}`}function L(e){return e.map(t=>t.slice())}function on(e){const t=[];for(let a=0;a<e;a++)t.push(Array.from({length:e},(i,o)=>Y(a===o?1:0)));return t}function rn(e,t){return e.map((a,i)=>a.concat(t[i]))}const ln=e=>e.length?e[0].length:0;function mn(e,t,a){return e.map(i=>i.slice(t,a))}function $(e){return e+1}function oe(e,t,a){const i=L(e),o=i.length,h=ln(i),m=Array.from({length:t},(r,z)=>z),c=[],s=[];let d=0,v=0,g=!1;const b=()=>L(i);s.push({kind:"init",matrix:b(),coeffCols:t,varOrder:m.slice(),caption:{en:"Initial augmented matrix.",hu:"Kiindulási kibővített mátrix."}});const T=r=>{let z=X;for(let I=0;I<t;I++){const u=i[r][I].abs();u.compare(z)>0&&(z=u)}return z},q=Math.min(o,t);for(let r=0;r<q;r++){if(a.pivoting==="partial"){let u=r;for(let x=r+1;x<o;x++)i[x][r].abs().compare(i[u][r].abs())>0&&(u=x);!W(i[u][r])&&u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,s.push({kind:"row-swap",matrix:b(),coeffCols:t,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Partial pivoting: swap rows ${$(r)} and ${$(u)} (largest |entry| in column ${$(r)}).`,hu:`Részleges főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje (legnagyobb |elem| a(z) ${$(r)}. oszlopban).`}}))}else if(a.pivoting==="scaled"){let u=r,x=null;for(let w=r;w<o;w++){const y=T(w),P=W(y)?X:i[w][r].abs().div(y);(x===null||P.compare(x)>0)&&(x=P,u=w)}!W(i[u][r])&&u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,s.push({kind:"row-swap",matrix:b(),coeffCols:t,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Scaled partial pivoting: swap rows ${$(r)} and ${$(u)} (largest scaled ratio).`,hu:`Skálázott főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje (legnagyobb skálázott hányados).`}}))}else if(a.pivoting==="complete"){let u=r,x=r;for(let w=r;w<o;w++)for(let y=r;y<t;y++)i[w][y].abs().compare(i[u][x].abs())>0&&(u=w,x=y);if(!W(i[u][x])&&(u!==r&&([i[r],i[u]]=[i[u],i[r]],d++,s.push({kind:"row-swap",matrix:b(),coeffCols:t,varOrder:m.slice(),swapRows:[r,u],caption:{en:`Complete pivoting: swap rows ${$(r)} and ${$(u)}.`,hu:`Teljes főelemkiválasztás: ${$(r)}. és ${$(u)}. sor cseréje.`}})),x!==r)){for(let w=0;w<o;w++)[i[w][r],i[w][x]]=[i[w][x],i[w][r]];[m[r],m[x]]=[m[x],m[r]],v++,s.push({kind:"col-swap",matrix:b(),coeffCols:t,varOrder:m.slice(),swapCols:[r,x],caption:{en:`Complete pivoting: swap columns ${$(r)} and ${$(x)} (track variable order).`,hu:`Teljes főelemkiválasztás: ${$(r)}. és ${$(x)}. oszlop cseréje (a változók sorrendjét követjük).`}})}}const z=i[r][r];if(c.push(z),W(z)){g=!0,s.push({kind:"pivot-select",matrix:b(),coeffCols:t,varOrder:m.slice(),pivot:[r,r],caption:{en:`Pivot a_{${$(r)}${$(r)}} = 0 and no nonzero entry is available — the elimination stops (matrix is singular${a.pivoting==="none"?" for this strategy":""}).`,hu:`A főelem a_{${$(r)}${$(r)}} = 0, és nincs nemnulla elem — az elimináció megáll (a mátrix szinguláris${a.pivoting==="none"?" ennél a stratégiánál":""}).`}});break}s.push({kind:"pivot-select",matrix:b(),coeffCols:t,varOrder:m.slice(),pivot:[r,r],caption:{en:`Pivot a_{${$(r)}${$(r)}} = ${j(z)}.`,hu:`Főelem a_{${$(r)}${$(r)}} = ${j(z)}.`}});const I=a.method==="gauss-jordan"?0:r+1;for(let u=I;u<o;u++){if(u===r||W(i[u][r]))continue;const x=i[u][r].div(z),w=[];for(let y=r;y<h;y++){const P=i[u][y];i[u][y]=i[u][y].sub(x.mul(i[r][y])),i[u][y].equals(P)||w.push([u,y])}s.push({kind:"eliminate",matrix:b(),coeffCols:t,varOrder:m.slice(),pivot:[r,r],multiplier:x,changed:w,caption:{en:`Eliminate: R${$(u)} ← R${$(u)} − (${j(x)})·R${$(r)}.`,hu:`Kiküszöbölés: S${$(u)} ← S${$(u)} − (${j(x)})·S${$(r)}.`}})}}if(a.method==="gauss-jordan"&&!g)for(let r=0;r<q;r++){const z=i[r][r];if(W(z)||z.equals(be))continue;const I=[];for(let u=0;u<h;u++)i[r][u]=i[r][u].div(z),I.push([r,u]);s.push({kind:"normalize",matrix:b(),coeffCols:t,varOrder:m.slice(),pivot:[r,r],changed:I,caption:{en:`Normalise: R${$(r)} ← R${$(r)} / (${j(z)}).`,hu:`Normálás: S${$(r)} ← S${$(r)} / (${j(z)}).`}})}return{steps:s,matrix:i,coeffCols:t,varOrder:m,pivots:c,rowSwaps:d,colSwaps:v,singular:g}}function pe(e){if(e.coeffCols!==e.matrix.length)return null;if(e.singular)return X;let t=be;for(const a of e.pivots)t=t.mul(a);return(e.rowSwaps+e.colSwaps)%2===1&&(t=t.neg()),t}function hn(e){const{matrix:t,coeffCols:a,varOrder:i,singular:o}=e;if(o)return{steps:[],solution:null};const h=a,m=new Array(a),c=[];for(let d=a-1;d>=0;d--){let v=t[d][h];for(let b=d+1;b<a;b++)v=v.sub(t[d][b].mul(m[b]));m[d]=v.div(t[d][d]);const g=i[d]+1;c.push({kind:"back-sub",matrix:L(t),coeffCols:a,varOrder:i.slice(),pivot:[d,d],caption:{en:`Back-substitution: x${g} = ${j(m[d])}.`,hu:`Visszahelyettesítés: x${g} = ${j(m[d])}.`}})}const s=new Array(a);for(let d=0;d<a;d++)s[i[d]]=m[d];return{steps:c,solution:s}}function dn(e,t){return e.map((a,i)=>a.concat([t[i]]))}function un(e,t,a){const i=e[0].length,o=dn(e,t),h=oe(o,i,a),m=pe(h);if(h.singular)return{steps:h.steps,solution:null,singular:!0,determinant:m};let c=h.steps,s;if(a.method==="gauss"){const v=hn(h);c=c.concat(v.steps),s=v.solution}else{const v=i;s=new Array(i);for(let g=0;g<i;g++)s[h.varOrder[g]]=h.matrix[g][v]}const d=s.map((v,g)=>`x${g+1} = ${j(v)}`).join(",  ");return c=c.concat([{kind:"done",matrix:L(h.matrix),coeffCols:i,varOrder:h.varOrder.slice(),caption:{en:`Solution: ${d}.`,hu:`Megoldás: ${d}.`}}]),{steps:c,solution:s,singular:!1,determinant:m}}function cn(e,t="none"){const a=e.length,i=rn(e,on(a)),o=oe(i,a,{method:"gauss-jordan",pivoting:t});if(o.singular)return{steps:o.steps,inverse:null,singular:!0};const h=mn(o.matrix,a,2*a);return{steps:o.steps.concat([{kind:"done",matrix:L(o.matrix),coeffCols:a,varOrder:o.varOrder.slice(),caption:{en:"The right block is A⁻¹.",hu:"A jobb oldali blokk az A⁻¹."}}]),inverse:h,singular:!1}}function fn(e,t="partial"){const a=e.length,i=oe(e.map(m=>m.slice()),a,{method:"gauss",pivoting:t}),o=pe(i);return{steps:i.steps,determinant:o}}function F(e,t,a,i,o){const h=t.length,m=[];for(let c=0;c<h;c++){const s=new Array(h+1).fill(X);s[c]=t[c],c<h-1&&(s[c+1]=a[c]),c>0&&(s[c-1]=c<=o?X:e[c-1]),s[h]=i[c],m.push(s)}return m}function bn(e){const t=e.d.length,a=e.a.slice(),i=e.d.slice(),o=e.c.slice(),h=e.b.slice(),m=[];m.push({kind:"init",matrix:F(a,i,o,h,0),coeffCols:t,varOrder:Array.from({length:t},(s,d)=>d),caption:{en:"Initial tridiagonal system (A | b).",hu:"Kiindulási tridiagonális rendszer (A | b)."}});for(let s=1;s<t;s++){if(W(i[s-1]))return m.push({kind:"pivot-select",matrix:F(a,i,o,h,s-1),coeffCols:t,varOrder:Array.from({length:t},(v,g)=>g),pivot:[s-1,s-1],caption:{en:`Zero pivot d${s} — the Thomas algorithm cannot continue without pivoting.`,hu:`Nulla főelem d${s} — a Thomas-algoritmus főelemkiválasztás nélkül nem folytatható.`}}),{steps:m,solution:null,singular:!0};const d=a[s-1].div(i[s-1]);i[s]=i[s].sub(d.mul(o[s-1])),h[s]=h[s].sub(d.mul(h[s-1])),m.push({kind:"eliminate",matrix:F(a,i,o,h,s),coeffCols:t,varOrder:Array.from({length:t},(v,g)=>g),pivot:[s-1,s-1],multiplier:d,changed:[[s,s-1],[s,s],[s,t]],caption:{en:`Eliminate a${s}: m = a${s}/d${s} = ${j(d)}; update d${s+1} and b${s+1}.`,hu:`a${s} kiküszöbölése: m = a${s}/d${s} = ${j(d)}; d${s+1} és b${s+1} frissítése.`}})}if(W(i[t-1]))return{steps:m,solution:null,singular:!0};const c=new Array(t);c[t-1]=h[t-1].div(i[t-1]),m.push({kind:"back-sub",matrix:F(a,i,o,h,t-1),coeffCols:t,varOrder:Array.from({length:t},(s,d)=>d),pivot:[t-1,t-1],caption:{en:`Back-substitution: x${t} = b${t}/d${t} = ${j(c[t-1])}.`,hu:`Visszahelyettesítés: x${t} = b${t}/d${t} = ${j(c[t-1])}.`}});for(let s=t-2;s>=0;s--)c[s]=h[s].sub(o[s].mul(c[s+1])).div(i[s]),m.push({kind:"back-sub",matrix:F(a,i,o,h,t-1),coeffCols:t,varOrder:Array.from({length:t},(d,v)=>v),pivot:[s,s],caption:{en:`Back-substitution: x${s+1} = (b${s+1} − c${s+1}·x${s+2})/d${s+1} = ${j(c[s])}.`,hu:`Visszahelyettesítés: x${s+1} = (b${s+1} − c${s+1}·x${s+2})/d${s+1} = ${j(c[s])}.`}});return{steps:m,solution:c,singular:!1}}const ae=[{id:"ex3-22",ref:"Example 3.22",name:{en:"Gaussian elimination",hu:"Gauss-elimináció"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[-11,-8,27,28],note:{en:"Solves cleanly without pivoting; solution (-3, 2, 4, -2).",hu:"Főelemkiválasztás nélkül is megoldható; megoldás (-3, 2, 4, -2)."}},{id:"ex3-24",ref:"Example 3.24 / 3.27",name:{en:"Zero pivot (needs pivoting)",hu:"Nulla főelem (kell csere)"},A:[[2,-1,0,-3],[2,-1,1,5],[-3,1,1,-2],[2,4,0,-1]],b:[8,2,-5,21],note:{en:"Without pivoting it stalls on a zero pivot. Switch pivoting to Partial to finish — solution (4, 3, 2, -1).",hu:"Főelemkiválasztás nélkül nulla főelemen elakad. Válts Részleges főelemkiválasztásra — megoldás (4, 3, 2, -1)."}},{id:"ex3-25",ref:"Example 3.25",name:{en:"Ill-conditioned 2×2",hu:"Rosszul kondicionált 2×2"},A:[["1/5000","-61/2"],["253/50","-21/20"]],b:["-6099/100","2509/10"],note:{en:"Exact solution (50, 2). With finite-digit arithmetic, dividing by the tiny pivot 0.0002 blows up the error — partial pivoting fixes it.",hu:"Pontos megoldás (50, 2). Véges jegyű aritmetikával a kicsi 0.0002 főelemmel való osztás felnagyítja a hibát — a részleges főelemkiválasztás megoldja."}},{id:"ex3-33a",ref:"§3.3 Exercise 1(a)",name:{en:"Exercise 3×3",hu:"Feladat 3×3"},A:[[2,2,-2],[-1,3,0],[4,2,-3]],b:[-4,-11,-1]}],pn=[{id:"ex3-38",ref:"Example 3.38",name:{en:"Invert a 3×3",hu:"3×3 invertálása"},A:[[1,0,2],[-1,1,0],[-2,0,-1]],b:[0,0,0],note:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]].",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]]."}},{id:"ex3-39",ref:"Example 3.39",name:{en:"Determinant = 114",hu:"Determináns = 114"},A:[[1,-2,-2,-2],[2,-1,2,4],[-1,2,3,-4],[-2,1,4,-2]],b:[0,0,0,0],note:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"ex3-7-1a",ref:"§3.7 Exercise 1(a)",name:{en:"Invert 3×3 (exercise)",hu:"3×3 invertálása (feladat)"},A:[[-1,1,2],[-2,1,0],[0,1,-1]],b:[0,0,0]}],B=[{id:"ex3-5-1",ref:"§3.5 Exercise 1",name:{en:"6×6 tridiagonal",hu:"6×6 tridiagonális"},a:["1/2","1/2","1/2","1/2","1/2"],d:[1,4,2,4,2,1],c:["-1/2","-1/2","-1/2","-1/2","-1/2"],b:["3/2",-4,2,-4,2,"-1/2"]},{id:"tri-small",ref:"Warm-up",name:{en:"4×4 tridiagonal",hu:"4×4 tridiagonális"},a:[-1,-1,-1],d:[4,4,4,4],c:[-1,-1,-1],b:[5,5,10,23]}];function $n(e,t,a){return!!e&&e[0]===t&&e[1]===a}function $e({matrix:e,coeffCols:t,pivot:a,changed:i,varOrder:o,bracket:h=!0}){const m=new Set((i??[]).map(([s,d])=>`${s},${d}`)),c=o!==void 0&&o.some((s,d)=>s!==d);return n.jsxs("div",{className:"matrix-view",children:[n.jsx("table",{className:`matrix${h?" bracketed":""}`,children:n.jsx("tbody",{children:e.map((s,d)=>n.jsx("tr",{children:s.map((v,g)=>{const b=[];return $n(a,d,g)?b.push("cell-pivot"):m.has(`${d},${g}`)&&b.push("cell-changed"),t!==void 0&&g===t&&b.push("rhs-sep"),n.jsx("td",{className:b.join(" ")||void 0,children:n.jsx(J,{tex:ie(v)})},g)})},d))})}),c&&n.jsx("div",{className:"var-order",children:o.map((s,d)=>n.jsxs("span",{children:["col ",d+1," → ",n.jsx(J,{tex:`x_{${s+1}}`})]},d))})]})}function gn({values:e,onChange:t,label:a,invalid:i}){var h;const o=((h=e[0])==null?void 0:h.length)??0;return n.jsxs("label",{className:"field",children:[a,n.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:`repeat(${o}, auto)`},children:e.map((m,c)=>m.map((s,d)=>n.jsx("input",{type:"text",inputMode:"decimal",value:s,"aria-label":`a${c+1}${d+1}`,style:i!=null&&i(c,d)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:v=>t(c,d,v.target.value)},`${c}-${d}`)))})]})}function U({values:e,onChange:t,label:a,invalid:i,horizontal:o=!1}){return n.jsxs("label",{className:"field",children:[a,n.jsx("div",{className:"matrix-input",style:{gridTemplateColumns:o?`repeat(${e.length}, auto)`:"auto"},children:e.map((h,m)=>n.jsx("input",{type:"text",inputMode:"decimal",value:h,"aria-label":`b${m+1}`,style:i!=null&&i(m)?{borderColor:"var(--err)",color:"var(--err)"}:void 0,onChange:c=>t(m,c.target.value)},m))})]})}const he=e=>e.map(t=>t.map(String)),G=e=>e.map(String),ge=e=>Array.from({length:e},()=>"0"),kn=e=>Array.from({length:e},()=>ge(e)),H=e=>e.map(t=>Y(t.trim()===""?0:t.trim())),vn=e=>e.map(H);function R(e){return e==="solve"?ae:pn}function xn({initial:e}){const{t,pick:a,lang:i}=S(),[o,h]=A.useState((e==null?void 0:e.mode)??"solve"),[m,c]=A.useState((e==null?void 0:e.method)??"gauss"),[s,d]=A.useState((e==null?void 0:e.pivot)??"none"),v=A.useMemo(()=>{const l=(e==null?void 0:e.mode)??"solve";if(l==="tridiagonal")return{presetId:(B.find(_=>_.id===(e==null?void 0:e.presetId))??B[0]).id};const k=R(l);return{presetId:(k.find(p=>p.id===(e==null?void 0:e.presetId))??k[0]).id}},[e==null?void 0:e.mode,e==null?void 0:e.presetId]),[g,b]=A.useState(v.presetId),T=ae[0],[q,r]=A.useState(()=>{const l=(e==null?void 0:e.mode)??"solve",k=R(l),f=k.find(p=>p.id===(e==null?void 0:e.presetId))??k[0]??T;return he(f.A)}),[z,I]=A.useState(()=>{const l=ae.find(k=>k.id===(e==null?void 0:e.presetId))??T;return G(l.b)}),[u,x]=A.useState(()=>{const l=B.find(k=>k.id===(e==null?void 0:e.presetId))??B[0];return{a:G(l.a),d:G(l.d),c:G(l.c),b:G(l.b)}}),[w,y]=A.useState(0),[P,Z]=A.useState(!1);function ee(l,k){if(b(k),l==="tridiagonal"){const _=B.find(V=>V.id===k)??B[0];x({a:G(_.a),d:G(_.d),c:G(_.c),b:G(_.b)});return}const f=R(l),p=f.find(_=>_.id===k)??f[0];p&&(r(he(p.A)),I(G(p.b)))}function ke(l){h(l),l==="inverse"?(c("gauss-jordan"),(s==="complete"||s==="scaled")&&d("none")):l==="determinant"&&c("gauss"),l==="tridiagonal"?ee(l,B[0].id):ee(l,R(l)[0].id)}function ve(l){r(k=>{const f=kn(l);for(let p=0;p<Math.min(l,k.length);p++)for(let _=0;_<Math.min(l,k[p].length);_++)f[p][_]=k[p][_];return f}),I(k=>{const f=ge(l);for(let p=0;p<Math.min(l,k.length);p++)f[p]=k[p];return f}),b("custom")}const C=A.useMemo(()=>{try{if(o==="tridiagonal"){const f=H(u.a),p=H(u.d),_=H(u.c),V=H(u.b);if(f.length!==p.length-1||_.length!==p.length-1||V.length!==p.length)return{steps:[],error:"dimension mismatch"};const D=bn({a:f,d:p,c:_,b:V});return{steps:D.steps,error:null,solution:D.solution,singular:D.singular}}const l=vn(q);if(o==="solve"){const f=un(l,H(z),{method:m,pivoting:s});return{steps:f.steps,error:null,solution:f.solution,determinant:f.determinant,singular:f.singular}}if(o==="inverse"){const f=cn(l,s==="partial"?"partial":"none");return{steps:f.steps,error:null,inverse:f.inverse,singular:f.singular}}const k=fn(l,s);return{steps:k.steps,error:null,determinant:k.determinant}}catch{return{steps:[],error:"invalid"}}},[o,m,s,q,z,u]);A.useEffect(()=>{y(0),Z(!1)},[C]);const te=A.useRef(null);A.useEffect(()=>{if(P)return te.current=window.setInterval(()=>{y(l=>l>=C.steps.length-1?(Z(!1),l):l+1)},900),()=>{te.current&&window.clearInterval(te.current)}},[P,C.steps.length]);const E=C.steps,N=Math.min(w,Math.max(0,E.length-1)),O=E[N],xe=q.length,re=o==="solve",le=o==="solve"||o==="determinant"||o==="inverse",_e=o==="inverse"?["none","partial"]:["none","partial","complete","scaled"],me=o==="tridiagonal"?B:R(o),K=me.find(l=>l.id===g)??null;return n.jsxs("div",{className:"grid-2",children:[n.jsxs("div",{className:"card stack",children:[n.jsxs("div",{className:"row",children:[n.jsxs("label",{className:"field",style:{flex:1},children:[t("lab.mode"),n.jsxs("select",{value:o,onChange:l=>ke(l.target.value),children:[n.jsx("option",{value:"solve",children:t("lab.mode.solve")}),n.jsx("option",{value:"inverse",children:t("lab.mode.inverse")}),n.jsx("option",{value:"determinant",children:t("lab.mode.determinant")}),n.jsx("option",{value:"tridiagonal",children:t("lab.mode.tridiagonal")})]})]}),n.jsxs("label",{className:"field",style:{flex:1},children:[t("lab.preset"),n.jsxs("select",{value:g,onChange:l=>ee(o,l.target.value),children:[me.map(l=>n.jsxs("option",{value:l.id,children:[l.ref," — ",a(l.name)]},l.id)),n.jsx("option",{value:"custom",children:t("lab.preset.custom")})]})]})]}),(re||le)&&n.jsxs("div",{className:"row",children:[re&&n.jsxs("label",{className:"field",style:{flex:1},children:[t("lab.method"),n.jsxs("select",{value:m,onChange:l=>c(l.target.value),children:[n.jsx("option",{value:"gauss",children:t("method.gauss")}),n.jsx("option",{value:"gauss-jordan",children:t("method.gaussJordan")})]})]}),le&&n.jsxs("label",{className:"field",style:{flex:1},children:[t("lab.pivoting"),n.jsx("select",{value:s,onChange:l=>d(l.target.value),children:_e.map(l=>n.jsx("option",{value:l,children:t(`pivot.${l}`)},l))})]})]}),K&&"note"in K&&K.note&&n.jsx("div",{className:"caption",children:a(K.note)}),o==="inverse"&&n.jsx("div",{className:"muted",style:{fontSize:"0.82rem"},children:t("lab.note.inverse")}),o==="tridiagonal"?n.jsxs("div",{className:"stack",children:[n.jsx(U,{label:i==="hu"?"Főátló d":"Diagonal d",values:u.d,horizontal:!0,onChange:(l,k)=>{b("custom"),x(f=>({...f,d:f.d.map((p,_)=>_===l?k:p)}))}}),n.jsx(U,{label:i==="hu"?"Felső átló c":"Super-diagonal c",values:u.c,horizontal:!0,onChange:(l,k)=>{b("custom"),x(f=>({...f,c:f.c.map((p,_)=>_===l?k:p)}))}}),n.jsx(U,{label:i==="hu"?"Alsó átló a":"Sub-diagonal a",values:u.a,horizontal:!0,onChange:(l,k)=>{b("custom"),x(f=>({...f,a:f.a.map((p,_)=>_===l?k:p)}))}}),n.jsx(U,{label:t("lab.vectorB"),values:u.b,horizontal:!0,onChange:(l,k)=>{b("custom"),x(f=>({...f,b:f.b.map((p,_)=>_===l?k:p)}))}})]}):n.jsxs("div",{className:"stack",children:[n.jsxs("label",{className:"field",children:[t("lab.size"),n.jsx("select",{value:xe,onChange:l=>ve(Number(l.target.value)),children:[2,3,4,5].map(l=>n.jsxs("option",{value:l,children:[l," × ",l]},l))})]}),n.jsx(gn,{label:t("lab.matrixA"),values:q,onChange:(l,k,f)=>{b("custom"),r(p=>p.map((_,V)=>V===l?_.map((D,Ae)=>Ae===k?f:D):_))}}),o==="solve"&&n.jsx(U,{label:t("lab.vectorB"),values:z,onChange:(l,k)=>{b("custom"),I(f=>f.map((p,_)=>_===l?k:p))}})]})]}),n.jsx("div",{className:"card stack",children:C.error||!O?n.jsx("p",{className:"feedback err",children:i==="hu"?"Érvénytelen bemenet.":"Invalid input."}):n.jsxs(n.Fragment,{children:[n.jsx("div",{className:"caption",children:a(O.caption)}),n.jsx($e,{matrix:O.matrix,coeffCols:o==="inverse"?void 0:O.coeffCols,pivot:O.pivot,changed:O.changed,varOrder:O.varOrder}),n.jsxs("div",{className:"kbd-controls",children:[n.jsxs("button",{className:"icon-btn",onClick:()=>y(0),disabled:N===0,children:["⏮ ",t("lab.first")]}),n.jsxs("button",{className:"icon-btn",onClick:()=>y(l=>Math.max(0,l-1)),disabled:N===0,children:["◀ ",t("lab.prev")]}),n.jsx("button",{className:"btn",onClick:()=>Z(l=>!l),children:P?`⏸ ${t("lab.pause")}`:`▶ ${t("lab.play")}`}),n.jsxs("button",{className:"icon-btn",onClick:()=>y(l=>Math.min(E.length-1,l+1)),disabled:N>=E.length-1,children:[t("lab.next")," ▶"]}),n.jsxs("button",{className:"icon-btn",onClick:()=>y(E.length-1),disabled:N>=E.length-1,children:[t("lab.last")," ⏭"]})]}),n.jsxs("div",{className:"row",children:[n.jsx("span",{className:"muted",style:{fontSize:"0.85rem",minWidth:110},children:t("lab.stepOf",{a:N+1,b:E.length})}),n.jsx("div",{className:"progress",children:n.jsx("div",{style:{width:`${(N+1)/E.length*100}%`}})})]}),n.jsx(_n,{run:C,mode:o})]})})]})}function _n({run:e,mode:t}){const{t:a}=S();return e.singular&&t!=="determinant"?n.jsx("div",{className:"caption",children:a("lab.singular")}):n.jsxs("div",{className:"card",style:{background:"var(--bg-sunken)"},children:[n.jsx("div",{className:"section-eyebrow",children:a("lab.result")}),(t==="solve"||t==="tridiagonal")&&e.solution&&n.jsxs("div",{className:"result-line",style:{marginTop:8},children:[n.jsxs("strong",{children:[a("lab.solution"),":"]}),e.solution.map((i,o)=>n.jsx("span",{className:"pill",children:n.jsx(J,{tex:`x_{${o+1}} = ${ie(i)}`})},o))]}),(t==="solve"||t==="determinant")&&e.determinant&&n.jsxs("div",{className:"result-line",style:{marginTop:8},children:[n.jsxs("strong",{children:[a("lab.determinant"),":"]}),n.jsx("span",{className:"pill",children:n.jsx(J,{tex:`\\det(A) = ${ie(e.determinant)}`})})]}),t==="inverse"&&e.inverse&&n.jsxs("div",{style:{marginTop:8},children:[n.jsxs("strong",{children:[a("lab.inverse"),":"]}),n.jsx($e,{matrix:e.inverse})]})]})}const Q=[{id:"q-tri-solve",kind:"vector",prompt:{en:"Solve this triangular system by backward substitution (enter x₁, x₂, x₃, x₄):",hu:"Oldd meg ezt a háromszög rendszert visszahelyettesítéssel (add meg x₁, x₂, x₃, x₄):"},tex:"\\begin{array}{rcrcrcrcr} 2x_1 &-& x_2 &+& 3x_3 &+& x_4 &=& 3\\\\ && 3x_2 &-& x_3 &+& 2x_4 &=& 13\\\\ &&&& 2x_3 &-& x_4 &=& -2\\\\ &&&&&& 3x_4 &=& 12 \\end{array}",answer:["-1","2","1","4"],solution:{en:"x₄ = 4, then x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1.",hu:"x₄ = 4, majd x₃ = (−2+4)/2 = 1, x₂ = (13+1−8)/3 = 2, x₁ = (3+2−3−4)/2 = −1."}},{id:"q-det-114",kind:"numeric",prompt:{en:"What is det(A) for the coefficient matrix of Example 3.39 (pivots 1, 3, 1, 38, no swaps)?",hu:"Mennyi det(A) a 3.39. példa együtthatómátrixára (főelemek 1, 3, 1, 38, csere nélkül)?"},answer:"114",solution:{en:"det(A) = product of pivots = 1·3·1·38 = 114.",hu:"det(A) = a főelemek szorzata = 1·3·1·38 = 114."}},{id:"q-dd-invertible",kind:"truefalse",prompt:{en:"Every (row) diagonally dominant matrix is invertible.",hu:"Minden (soronként) diagonálisan domináns mátrix invertálható."},answer:!0,solution:{en:"True. Diagonal dominance implies Ax = 0 has only the trivial solution, so A is nonsingular.",hu:"Igaz. A diagonális dominanciából következik, hogy Ax = 0-nak csak triviális megoldása van, így A reguláris."}},{id:"q-partial-pivot",kind:"choice",prompt:{en:"In column 1 the entries (top to bottom) are 2, 2, −3, 2. Which row does partial pivoting move to the top?",hu:"Az 1. oszlop elemei (fentről le) 2, 2, −3, 2. Melyik sort viszi felülre a részleges főelemkiválasztás?"},choices:[{en:"Row 1 (value 2)",hu:"1. sor (érték 2)"},{en:"Row 3 (value −3)",hu:"3. sor (érték −3)"},{en:"The first nonzero row",hu:"Az első nemnulla sor"},{en:"No swap is needed",hu:"Nem kell csere"}],answer:1,solution:{en:"Partial pivoting picks the largest magnitude, |−3| = 3, so row 3 moves up.",hu:"A részleges főelemkiválasztás a legnagyobb abszolút értéket választja, |−3| = 3, így a 3. sor kerül felülre."}},{id:"q-gauss-cost",kind:"choice",prompt:{en:"What is the leading-order operation count of Gaussian elimination?",hu:"Mi a Gauss-elimináció vezető rendű műveletigénye?"},choices:[{en:"n²/2",hu:"n²/2"},{en:"n³/3",hu:"n³/3"},{en:"n³/2",hu:"n³/2"},{en:"5n − 4",hu:"5n − 4"}],answer:1,solution:{en:"Forward elimination plus back-substitution is n³/3 + O(n²) multiplications/divisions.",hu:"Az előre elimináció és a visszahelyettesítés együtt n³/3 + O(n²) szorzás/osztás."}},{id:"q-gj-cost",kind:"truefalse",prompt:{en:"Gauss–Jordan elimination is asymptotically cheaper than Gaussian elimination.",hu:"A Gauss–Jordan-elimináció aszimptotikusan olcsóbb, mint a Gauss-elimináció."},answer:!1,solution:{en:"False. Gauss–Jordan costs ≈ n³/2, more than Gaussian elimination’s n³/3.",hu:"Hamis. A Gauss–Jordan költsége ≈ n³/2, több, mint a Gauss-elimináció n³/3-a."}},{id:"q-thomas-cost",kind:"choice",prompt:{en:"How many multiplications/divisions does the Thomas (tridiagonal) algorithm need?",hu:"Hány szorzás/osztás kell a Thomas- (tridiagonális) algoritmushoz?"},choices:[{en:"n³/3",hu:"n³/3"},{en:"n²/2",hu:"n²/2"},{en:"5n − 4",hu:"5n − 4"},{en:"2ⁿ",hu:"2ⁿ"}],answer:2,solution:{en:"A tridiagonal solve is linear: 5n − 4 multiplications/divisions.",hu:"A tridiagonális megoldás lineáris: 5n − 4 szorzás/osztás."}},{id:"q-inv-entry",kind:"numeric",prompt:{en:"For A of Example 3.38, what is the (1,1) entry of A⁻¹? (Enter a fraction like -1/3.)",hu:"A 3.38. példa A mátrixára mi az A⁻¹ (1,1) eleme? (Adj meg törtet, pl. -1/3.)"},answer:"-1/3",solution:{en:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], so the (1,1) entry is −1/3.",hu:"A⁻¹ = (1/3)·[[-1,0,-2],[-1,3,-2],[2,0,1]], tehát az (1,1) elem −1/3."}},{id:"q-posdef",kind:"choice",prompt:{en:"A symmetric matrix is positive definite if and only if…",hu:"Egy szimmetrikus mátrix akkor és csak akkor pozitív definit, ha…"},choices:[{en:"all its entries are positive",hu:"minden eleme pozitív"},{en:"it is diagonally dominant",hu:"diagonálisan domináns"},{en:"all leading principal minors are positive",hu:"minden bal felső főminora pozitív"},{en:"its determinant is positive",hu:"a determinánsa pozitív"}],answer:2,solution:{en:"Sylvester’s criterion: positive definite ⇔ every leading principal minor is positive.",hu:"Sylvester-kritérium: pozitív definit ⇔ minden bal felső főminor pozitív."}}];function de(e,t){try{return Y(e.trim()).equals(Y(t.trim()))}catch{return!1}}function ue(e,t){switch(e.kind){case"choice":return t.choice===e.answer;case"truefalse":return t.bool===e.answer;case"numeric":return de(t.text??"",e.answer);case"vector":{const a=(t.text??"").split(/[,\s]+/).filter(Boolean);return a.length!==e.answer.length?!1:a.every((i,o)=>de(i,e.answer[o]))}}}function An({item:e,onAnswered:t}){const{t:a,pick:i}=S(),[o,h]=A.useState({}),[m,c]=A.useState(!1),[s,d]=A.useState(!1),v=m&&ue(e,o);function g(){c(!0),t(ue(e,o))}return n.jsxs("div",{className:"card stack",children:[n.jsxs("div",{children:[n.jsx("span",{className:"section-eyebrow",children:a("quiz.title")}),n.jsx("p",{style:{marginBottom:6},children:i(e.prompt)}),e.tex&&n.jsx(J,{tex:e.tex,block:!0})]}),e.kind==="choice"&&n.jsx("div",{className:"choices",children:e.choices.map((b,T)=>{let q="choice";return o.choice===T&&(q+=" selected"),m&&T===e.answer&&(q+=" correct"),m&&o.choice===T&&T!==e.answer&&(q+=" wrong"),n.jsx("button",{className:q,onClick:()=>!m&&h({choice:T}),disabled:m,children:i(b)},T)})}),e.kind==="truefalse"&&n.jsxs("div",{className:"seg",children:[n.jsx("button",{className:o.bool===!0?"active":"",onClick:()=>!m&&h({bool:!0}),children:a("quiz.true")}),n.jsx("button",{className:o.bool===!1?"active":"",onClick:()=>!m&&h({bool:!1}),children:a("quiz.false")})]}),(e.kind==="numeric"||e.kind==="vector")&&n.jsx("input",{type:"text",placeholder:e.kind==="vector"?"x₁, x₂, …":a("quiz.placeholder"),value:o.text??"",onChange:b=>h({text:b.target.value}),disabled:m,style:{maxWidth:280}}),n.jsxs("div",{className:"row",children:[m?n.jsx("span",{className:v?"feedback ok":"feedback err",children:a(v?"quiz.correct":"quiz.incorrect")}):n.jsx("button",{className:"btn",onClick:g,children:a("quiz.check")}),n.jsx("button",{className:"btn secondary",onClick:()=>d(b=>!b),children:a(s?"quiz.hideSolution":"quiz.showSolution")})]}),s&&n.jsx("div",{className:"caption",children:i(e.solution)})]})}function yn(){const{t:e}=S(),[t,a]=A.useState(0),[i,o]=A.useState({}),h=Q[t],m=Object.values(i).filter(Boolean).length;return n.jsxs("div",{className:"stack",children:[n.jsxs("div",{className:"row",style:{justifyContent:"space-between"},children:[n.jsx("span",{className:"muted",children:e("quiz.questionOf",{a:t+1,b:Q.length})}),n.jsxs("span",{className:"pill",children:[e("quiz.score"),": ",m," / ",Object.keys(i).length]})]}),n.jsx(An,{item:h,onAnswered:c=>o(s=>({...s,[t]:c}))},h.id),n.jsxs("div",{className:"row",children:[n.jsxs("button",{className:"icon-btn",onClick:()=>a(c=>Math.max(0,c-1)),disabled:t===0,children:["◀ ",e("quiz.prev")]}),n.jsxs("button",{className:"icon-btn",onClick:()=>a(c=>Math.min(Q.length-1,c+1)),disabled:t===Q.length-1,children:[e("quiz.next")," ▶"]})]})]})}const wn=[...fe.map(e=>({id:e.id,no:e.number,title:e.title,blurb:e.summary})),{id:"lab",no:"3·lab",title:{en:"Elimination Lab",hu:"Eliminációs labor"},blurb:{en:"Step through Gaussian / Gauss–Jordan elimination interactively.",hu:"Lépkedj végig a Gauss- / Gauss–Jordan-eliminációi lépéseken."}},{id:"quiz",no:"3·quiz",title:{en:"Quiz",hu:"Kvíz"},blurb:{en:"Check your understanding.",hu:"Ellenőrizd a tudásod."}}];function Gn(){const{t:e,lang:t}=S(),a=we();return A.useEffect(()=>{let i=decodeURIComponent(a.hash.replace(/^#/,""));if(!i){const o=a.pathname.match(/\/lessons\/([^/]+)/);o?i=o[1]:/\/lab$/.test(a.pathname)?i="lab":/\/quiz$/.test(a.pathname)&&(i="quiz")}i&&requestAnimationFrame(()=>{var o;return(o=document.getElementById(i))==null?void 0:o.scrollIntoView()})},[a.pathname,a.hash]),n.jsxs("div",{className:"app-shell ch-linear-systems",children:[n.jsx(je,{sections:wn}),n.jsx("main",{children:n.jsxs("div",{className:"container",children:[n.jsxs("section",{className:"hero",id:"top",children:[n.jsx("span",{className:"section-eyebrow",children:e("app.subtitle")}),n.jsx("h1",{children:e("app.title")}),n.jsx("p",{children:e("home.tagline")}),n.jsx("p",{style:{marginTop:4},children:e("home.lead")})]}),fe.map(i=>n.jsxs("section",{id:i.id,className:"ls-section",children:[n.jsx(Be,{section:i}),sn(i.id).map(o=>n.jsx(qe,{snippets:o.snippets,caption:o.caption},o.id))]},i.id)),n.jsxs("section",{id:"lab",className:"ls-section stack",children:[n.jsxs("div",{children:[n.jsx("span",{className:"section-eyebrow",children:e("nav.lab")}),n.jsx("h1",{style:{margin:"4px 0 2px"},children:e("nav.lab")})]}),n.jsx(xn,{initial:{}})]}),n.jsxs("section",{id:"quiz",className:"ls-section stack",children:[n.jsxs("div",{children:[n.jsx("span",{className:"section-eyebrow",children:e("nav.quiz")}),n.jsx("h1",{style:{margin:"4px 0 2px"},children:e("quiz.title")})]}),n.jsx(yn,{})]})]})}),n.jsx("footer",{className:"footer",children:n.jsx("div",{className:"container",children:t==="hu"?"Numerikus analízis · 3. fejezet — interaktív tananyag. F. Hartung nyomán, Pannon Egyetem.":"Numerical Analysis · Chapter 3 — interactive companion. After F. Hartung, University of Pannonia."})})]})}export{Gn as default};
