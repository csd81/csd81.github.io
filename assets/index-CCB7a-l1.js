...
`)}}}fences(n){const e=this.rules.block.fences.exec(n);if(e){const t=e[0],o=Xt(t,e[3]||"");return{type:"code",raw:t,lang:e[2]?e[2].trim().replace(this.rules.inline.anyPunctuation,"$1"):e[2],text:o}}}heading(n){const e=this.rules.block.heading.exec(n);if(e){let t=e[2].trim();if(/#$/.test(t)){const o=re(t,"#");(this.options.pedantic||!o||/ $/.test(o))&&(t=o.trim())}return{type:"heading",raw:e[0],depth:e[1].length,text:t,tokens:this.lexer.inline(t)}}}hr(n){const e=this.rules.block.hr.exec(n);if(e)return{type:"hr",raw:e[0]}}blockquote(n){const e=this.rules.block.blockquote.exec(n);if(e){let t=e[0].replace(/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,`
    $1`);t=re(t.replace(/^ *>[ \t]?/gm,""),`
`);const o=this.lexer.state.top;this.lexer.state.top=!0;const s=this.lexer.blockTokens(t);return this.lexer.state.top=o,{type:"blockquote",raw:e[0],tokens:s,text:t}}}list(n){let e=this.rules.block.list.exec(n);if(e){let t=e[1].trim();const o=t.length>1,s={type:"list",raw:"",ordered:o,start:o?+t.slice(0,-1):"",loose:!1,items:[]};t=o?`\\d{1,9}\\${t.slice(-1)}`:`\\${t}`,this.options.pedantic&&(t=o?t:"[*+-]");const i=new RegExp(`^( {0,3}${t})((?:[	 ][^\\n]*)?(?:\\n|$))`);let r="",l="",m=!1;for(;n;){let h=!1;if(!(e=i.exec(n))||this.rules.block.hr.test(n))break;r=e[0],n=n.substring(r.length);let c=e[2].split(`
`,1)[0].replace(/^\t+/,g=>" ".repeat(3*g.length)),p=n.split(`
`,1)[0],$=0;this.options.pedantic?($=2,l=c.trimStart()):($=e[2].search(/[^ ]/),$=$>4?1:$,l=c.slice($),$+=e[1].length);let d=!1;if(!c&&/^ *$/.test(p)&&(r+=p+`
`,n=n.substring(p.length+1),h=!0),!h){const g=new RegExp(`^ {0,${Math.min(3,$-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),v=new RegExp(`^ {0,${Math.min(3,$-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),z=new RegExp(`^ {0,${Math.min(3,$-1)}}(?:\`\`\`|~~~)`),w=new RegExp(`^ {0,${Math.min(3,$-1)}}#`);for(;n;){const A=n.split(`
`,1)[0];if(p=A,this.options.pedantic&&(p=p.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),z.test(p)||w.test(p)||g.test(p)||v.test(n))break;if(p.search(/[^ ]/)>=$||!p.trim())l+=`
`+p.slice($);else{if(d||c.search(/[^ ]/)>=4||z.test(c)||w.test(c)||v.test(c))break;l+=`
`+p}!d&&!p.trim()&&(d=!0),r+=A+`
`,n=n.substring(A.length+1),c=p.slice($)}}s.loose||(m?s.loose=!0:/\n *\n *$/.test(r)&&(m=!0));let f=null,x;this.options.gfm&&(f=/^\[[ xX]\] /.exec(l),f&&(x=f[0]!=="[ ] ",l=l.replace(/^\[[ xX]\] +/,""))),s.items.push({type:"list_item",raw:r,task:!!f,checked:x,loose:!1,text:l,tokens:[]}),s.raw+=r}s.items[s.items.length-1].raw=r.trimEnd(),s.items[s.items.length-1].text=l.trimEnd(),s.raw=s.raw.trimEnd();for(let h=0;h<s.items.length;h++)if(this.lexer.state.top=!1,s.items[h].tokens=this.lexer.blockTokens(s.items[h].text,[]),!s.loose){const c=s.items[h].tokens.filter($=>$.type==="space"),p=c.length>0&&c.some($=>/\n.*\n/.test($.raw));s.loose=p}if(s.loose)for(let h=0;h<s.items.length;h++)s.items[h].loose=!0;return s}}html(n){const e=this.rules.block.html.exec(n);if(e)return{type:"html",block:!0,raw:e[0],pre:e[1]==="pre"||e[1]==="script"||e[1]==="style",text:e[0]}}def(n){const e=this.rules.block.def.exec(n);if(e){const t=e[1].toLowerCase().replace(/\s+/g," "),o=e[2]?e[2].replace(/^<(.*)>$/,"$1").replace(this.rules.inline.anyPunctuation,"$1"):"",s=e[3]?e[3].substring(1,e[3].length-1).replace(this.rules.inline.anyPunctuation,"$1"):e[3];return{type:"def",tag:t,raw:e[0],href:o,title:s}}}table(n){const e=this.rules.block.table.exec(n);if(!e||!/[:|]/.test(e[2]))return;const t=Pe(e[1]),o=e[2].replace(/^\||\| *$/g,"").split("|"),s=e[3]&&e[3].trim()?e[3].replace(/\n[ \t]*$/,"").split(`
...
Please report this to https://github.com/markedjs/marked.`,n){const o="<p>An error occurred:</p><pre>"+S(t.message+"",!0)+"</pre>";return e?Promise.resolve(o):o}if(e)return Promise.reject(t);throw t}};const G=new _n;function y(a,n){return G.parse(a,n)}y.options=y.setOptions=function(a){return G.setOptions(a),y.defaults=G.defaults,Ue(y.defaults),y};y.getDefaults=we;y.defaults=X;y.use=function(...a){return G.use(...a),y.defaults=G.defaults,Ue(y.defaults),y};y.walkTokens=function(a,n){return G.walkTokens(a,n)};y.parseInline=G.parseInline;y.Parser=N;y.parser=N.parse;y.Renderer=ce;y.TextRenderer=Fe;y.Lexer=R;y.lexer=R.lex;y.Tokenizer=ue;y.Hooks=te;y.parse=y;y.options;y.setOptions;y.use;y.walkTokens;y.parseInline;N.parse;R.lex;const kn={delimiters:[{left:"$$",right:"$$",display:!0},{left:"$",right:"$",display:!1},{left:"\\(",right:"\\)",display:!1},{left:"\\[",right:"\\]",display:!0}],throwOnError:!1};function Ee(a){a&&Wt(a,kn)}y.setOptions({breaks:!0,gfm:!0});function Ie(a,n){const e=()=>n.forEach(o=>{if(o&&o.offsetParent!==null)try{ne.Plots.resize(o)}catch{}});requestAnimationFrame(e),setTimeout(e,60);let t;return typeof ResizeObserver<"u"&&(t=new ResizeObserver(e),t.observe(a)),()=>{t&&t.disconnect()}}function pe(a={}){const n=J("--fg")||"#1a1a2e",e=J("--grid")||"#d8d8e0",t=J("--surface")||"#ffffff";return{paper_bgcolor:t,plot_bgcolor:t,font:{color:n,family:"system-ui, sans-serif"},margin:{l:44,r:16,t:28,b:40},xaxis:{gridcolor:e,zerolinecolor:e},yaxis:{gridcolor:e,zerolinecolor:e},showlegend:!0,legend:{orientation:"h",y:1.12,x:0},...a}}const de=()=>J("--accent")||"#e63946",$e=()=>J("--point")||"#3a6ea5",fe={displayModeBar:!1,responsive:!0},he={xs:[-1,1,2.5,3,4,4.5,6],ys:[0,1.2,1.9,2.5,3.1,3.2,4.5]};function zn(a){let n=[...he.xs],e=[...he.ys],t=!1,o=.5,s=.5;a.innerHTML=`
    ...
    </div>`;const i=a.querySelector("[data-plot]"),r=a.querySelector("[data-readout]"),l=a.querySelector("[data-sliders]");function m(d,f){const x=(f-d)*.1||1;return[d-x,f+x]}function h(){const{a:d,b:f}=Q(n,e),[x,g]=m(Math.min(...n),Math.max(...n)),v=[{x:n,y:e,mode:"markers",type:"scatter",name:b({hu:"adatok",en:"data"}),marker:{size:11,color:$e()}},{x:[x,g],y:[d*x+f,d*g+f],mode:"lines",name:b(q.demoBestFit),line:{color:de(),width:3}}];t&&v.push({x:[x,g],y:[o*x+s,o*g+s],mode:"lines",name:b(q.demoGuess),line:{color:"#f4a261",width:2,dash:"dash"}}),ne.react(i,v,pe({xaxis:{range:[x,g]}}),fe);const z=D(A=>d*A+f,n,e);let w=`<div class="ro-row">$\\bar a = ${d.toFixed(6)},\\quad \\bar b = ${f.toFixed(6)}$</div>`;if(w+=`<div class="ro-row"><span class="ro-label">${b(q.optimalError)}:</span> $F = ${z.toFixed(6)}$</div>`,t){const A=D(F=>o*F+s,n,e),B=A>0?z/A:1,W=B>.98?"★★★":B>.85?"★★☆":B>.6?"★☆☆":"☆☆☆";w+=`<div class="ro-row"><span class="ro-label">${b(q.yourError)}:</span> $F = ${A.toFixed(6)}$ <span class="ro-stars">${W}</span></div>`}r.innerHTML=w,Ee(r)}function c(){const d=i.querySelector(".nsewdrag");if(!d)return;let f=-1;const x=g=>{const v=d.getBoundingClientRect(),z=i._fullLayout.xaxis,w=i._fullLayout.yaxis;return{x:z.p2d(g.clientX-v.left),y:w.p2d(g.clientY-v.top)}};d.addEventListener("mousedown",g=>{const v=x(g),z=i._fullLayout.xaxis,w=i._fullLayout.yaxis,A=z.range[1]-z.range[0],B=w.range[1]-w.range[0];let W=-1,F=1/0;for(let V=0;V<n.length;V++){const se=(n[V]-v.x)/A,Z=(e[V]-v.y)/B,K=se*se+Z*Z;K<F&&(F=K,W=V)}W>=0&&F<.0025&&(f=W,g.preventDefault())}),window.addEventListener("mousemove",g=>{f<0||(e[f]=x(g).y,h())}),window.addEventListener("mouseup",()=>{f=-1})}a.querySelector('[data-act="best"]').addEventListener("click",()=>{t=!1,l.hidden=!0,a.querySelector('[data-act="guess"]').setAttribute("aria-pressed","false"),h()}),a.querySelector('[data-act="guess"]').addEventListener("click",d=>{t=!t,l.hidden=!t,d.currentTarget.setAttribute("aria-pressed",String(t)),h()}),a.querySelector('[data-act="add"]').addEventListener("click",()=>{const d=Q(n,e),f=Math.max(...n)+1;n.push(f),e.push(d.a*f+d.b),h(),setTimeout(c,60)}),a.querySelector('[data-act="reset"]').addEventListener("click",()=>{n=[...he.xs],e=[...he.ys],h(),setTimeout(c,60)}),a.querySelectorAll("[data-slider]").forEach(d=>{d.value=d.dataset.slider==="a"?o:s,d.addEventListener("input",()=>{d.dataset.slider==="a"?o=parseFloat(d.value):s=parseFloat(d.value),a.querySelector("[data-ga]").textContent=o.toFixed(2),a.querySelector("[data-gb]").textContent=s.toFixed(2),h()})}),a.querySelector("[data-ga]").textContent=o.toFixed(2),a.querySelector("[data-gb]").textContent=s.toFixed(2),h(),setTimeout(c,80);const p=ve(()=>h()),$=Ie(a,[i]);return()=>{p(),$()}}const De={xs:[-1,0,.5,1,2,2.5,3],ys:[1.4,1.9,1.6,1.7,.2,-.1,-2]};function vn(a){let n=[...De.xs],e=[...De.ys],t=2;const o=Math.min(6,n.length-1);a.innerHTML=`
    <div class="demo">
      ...
      <div class="demo-plot" data-plot></div>
      <div class="demo-readout" data-readout></div>
    </div>`;const s=a.querySelector("[data-plot]"),i=a.querySelector("[data-readout]");a.querySelector("[data-deg]").textContent=t;function r(){const h=Ft(n,e,t),c=Math.min(...n)-.4,p=Math.max(...n)+.4,$=[],d=[],f=120;for(let z=0;z<=f;z++){const w=c+(p-c)*z/f;$.push(w),d.push(Be(h,w))}ne.react(s,[{x:n,y:e,mode:"markers",type:"scatter",name:b({hu:"adatok",en:"data"}),marker:{size:11,color:$e()}},{x:$,y:d,mode:"lines",name:`${b({hu:"fokszám",en:"degree"})} ${t}`,line:{color:de(),width:3}}],pe({xaxis:{range:[c,p]}}),fe);const x=D(z=>Be(h,z),n,e);let v=`<div class="ro-row">$p(x) = ${h.map((z,w)=>w===0?z.toFixed(4):`${z>=0?"+":""}${z.toFixed(4)}x^{${w}}`).join(" ")}$</div>`;v+=`<div class="ro-row"><span class="ro-label">${b(q.error)}:</span> $F = ${x.toFixed(6)}$</div>`,t>=n.length-1&&(v+=`<div class="ro-row ro-warn">${b({hu:"⚠ $m \\ge n$: interpoláció, a hiba ≈ 0.",en:"⚠ $m \\ge n$: interpolation, error ≈ 0."})}</div>`),i.innerHTML=v,Ee(i)}a.querySelector('[data-slider="m"]').addEventListener("input",h=>{t=parseInt(h.target.value,10),a.querySelector("[data-deg]").textContent=t,r()}),r();const l=ve(()=>r()),m=Ie(a,[s]);return()=>{l(),m()}}const wn={exp:{xs:[0,1,1.5,2,3,4],ys:[.3,.7,.9,1.2,1.8,2.7]},power:{xs:[.5,1,1.5,2.5,3],ys:[.7,1.1,1.6,2.1,2.3]}};function qn(a){let n="exp";a.innerHTML=`
    <div class="demo">
      <div class="demo-controls">
...
`,ui=`polyFit[t_, y_, degree_] := Module[{A},
