...
3. The Mechanics of Decomposition: Detail the process of decomposing matrix A into L, D, and U. Explain how this decomposition facilitates the transition from a standard linear system Ax=b to the iterative form x^{(k+1)} = Tx^{(k)} + c.
