...
5. Describe the risk of "overflow" when calculating expressions like 15^{40} / 40!. Overflow occurs when an intermediate or final result exceeds the maximum value a computer's arithmetic can represent. In this example, calculating 15^{40} or 40! separately produces numbers too large for single-precision storage, even though the final ratio is a very small number close to zero.
