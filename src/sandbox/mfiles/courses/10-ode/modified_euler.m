...
% MODIFIED_EULER  Módosított Euler-módszer (explicit középpont-szabály, RK2).
%   y = modified_euler(f, t0, y0, h, n)
...
%   n:  Lépések száma
    ...
    for i = 1:n
        k1 = f(t, y);
        % Meredekség becslése a részintervallum középpontjában (prediktor lépés)
        k2 = f(t + h/2, y + h/2 * k1);     
        
        % Új érték kiszámítása a középponti meredekséggel (korrektor lépés)
        y = y + h * k2;
        t = t + h;
    end
end
...
function modified_euler_demo()
    ...
    % Közelítő megoldás kiszámítása a módosított Euler-módszerrel
    y_approx = modified_euler(f, t0, y0, h, n);
