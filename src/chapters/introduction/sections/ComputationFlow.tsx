...
export function ComputationFlow() {
  ...
  const errTag = (k: ErrKey, label: string) => (
    <button
      ...
      style={{
        border: '1px solid var(--border)',
        background: hover === k ? 'var(--accent-soft)' : 'var(--panel)',
        color: hover === k ? 'var(--accent)' : 'var(--text)',
        cursor: 'pointer',
      }}
    >
      {label}
  ...
  return (
    <Section id="flow" tag={t('flow.tag')} title={t('flow.title')} lead={t('flow.lead')}>
      <div className="split">
        <div className="prose">
          <p>{t('flow.p1')}</p>
