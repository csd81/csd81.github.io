...
export default function CompareDemo({ presetId = "main", defaultMethods = ["euler", "rk4"] }) {
  ...
  // Formerly POSTed to /api/solve. The identical engine runs in the browser, so
  // we summarise the locally-computed `results` (each carries `maxError`).
  const callApi = () => {
    setApiBusy(true);
    if (!results.length) {
      setApiResult("⚠ " + (error || "no result"));
    } else {
      setApiResult(
        results.map((r) => `${r.key}: maxErr=${r.maxError.toExponential(3)}`).join("  ·  "),
      );
    }
    setApiBusy(false);
  };
