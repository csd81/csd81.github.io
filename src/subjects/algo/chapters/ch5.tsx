...
/* ════ Extended Euclidean / Bézout solver ════ */
function ExtendedEuclid() {
  const [a, setA] = useState(9867);
  const [b, setB] = useState(8855);
  const [c, setC] = useState(759);

  function extgcd(aa: number, bb: number): {
    gcd: number; u: number; v: number;
    steps: { i: number; q: number; r: number; xi: number; zeta: number }[];
  } {
    // single-pass Knuth variant (5.11 algorithm)
    let xi_m2 = 1, xi_m1 = 0;
    let ze_m2 = 0, ze_m1 = 1;
    const steps: { i: number; q: number; r: number; xi: number; zeta: number }[] = [];
    let a0 = Math.abs(aa), b0 = Math.abs(bb);
    let swapped = false;
    if (a0 < b0) { const tmp = a0; a0 = b0; b0 = tmp; swapped = true; }
    let idx = 1;
    while (b0 > 0) {
      const q = Math.floor(a0 / b0);
      const r = a0 % b0;
      const xi_new = xi_m2 - xi_m1 * q;
      const ze_new = ze_m2 - ze_m1 * q;
      steps.push({ i: idx, q, r, xi: xi_m1, zeta: ze_m1 });
      xi_m2 = xi_m1; xi_m1 = xi_new;
      ze_m2 = ze_m1; ze_m1 = ze_new;
      a0 = b0; b0 = r;
      idx++;
    }
    let u = xi_m2, v = ze_m2;
    if (swapped) { const tmp = u; u = v; v = tmp; }
    return { gcd: a0, u, v, steps };
  }

  const A = Math.abs(a), B = Math.abs(b);
  const { gcd, u, v, steps } = extgcd(A, B);
  const solvable = gcd > 0 && c % gcd === 0;
  const x0 = solvable ? u * (c / gcd) : 0;
  const y0 = solvable ? v * (c / gcd) : 0;
  const check = A * x0 + B * y0;

  return (
    <div className="info-box">
      <span className="lbl" style={{ color: '#a78bfa' }}>5.4 / 5.11 Kiterjesztett Euklideszi — ax + by = c megoldó</span>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table className="cayley" style={{ fontFamily: 'monospace', fontSize: '.82rem', width: '100%' }}>
          <thead>
            <tr>
              <th>i</th><th>q_i</th><th>r_i</th><th>ξ_i</th><th>ζ_i</th>
            </tr>
          </thead>
          <tbody>
            ...
            {steps.map((s, idx) => (
              <tr key={idx} style={s.r === 0 ? { background: 'rgba(167,139,250,.13)' } : undefined}>
                <td>{s.i}</td>
                <td>{s.q}</td>
                <td style={{ color: s.r === 0 ? '#fbbf24' : undefined, fontWeight: s.r === 0 ? 700 : undefined }}>{s.r}</td>
                <td style={{ color: '#a78bfa' }}>{s.xi}</td>
                <td style={{ color: '#a78bfa' }}>{s.zeta}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div style={{ fontSize: '.85rem', margin: '.4rem 0' }}>
      ...
      {solvable && (
        <div className="def-box" style={{ fontFamily: 'monospace', fontSize: '.85rem' }}>
          ...
          <br />
          Ellenőrzés: {A}·{x0} + {B}·{y0} = <strong style={{ color: '#fbbf24' }}>{check}</strong>
          {check === c ? ' ✓' : ' ✗'}
          <br />
...
/* ════ Tab 3: 5.6 Példa — visszaforgatás részletesen ════ */
const t3 = String.raw`
<div class="ex-box">
...
/* ════ Tab 4: Általános megoldás + 5.11 egyetlen menet ════ */
const t4 = String.raw`
<div class="thm-box">
...
const TABS: Tab[] = [
  ...
  { id: 'general', label: 'Általános megoldás', content: <div><RichTex html={t4} /><ExtendedEuclid /></div> },
  { id: 'three', label: '5.2 n-változós', content: <RichTex html={t5} /> },
];
